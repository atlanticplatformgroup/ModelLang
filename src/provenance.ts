...
function artifactRole(path: string): ArtifactProvenance["artifacts"][number]["role"] {
  ...
  if (["operations.json", "decisions.json", "capabilities.json", "agent-tools.json", "mcp.json", "ui.json", "semantic.json", "events.json", "openapi.json", "agent-plugin/plugin.json", "agent-plugin/mcp.json"].includes(path)) return "contract";
  if (["enforcement.json", "enforcement.md", "extensions.json", "target-capabilities.json", "sml-agent-assessment.json"].includes(path)) return "assurance";
  return "projection";
}
