...
export function generateFailureObserverInfrastructureStatements(ir: ModelIR): string[] {
  const internal = ir.model.naming.internalSchema;
  const outbox = qname(internal, "event_outbox");
  const failures = qname(internal, "consumer_failure");
  ...
  return [
    ...
    `      row_value.${quoteIdent("publication_recovery_mode")} = 'manual' AS recovery_eligible`,
    ...
    `      ${consumerRecoveryEligibility} AS recovery_eligible`,
...
function generateGrants(ir: ModelIR, includeApplicability = true, includeFailureObservation = false): string {
  const schema = ir.model.naming.sqlSchema;
  const internal = ir.model.naming.internalSchema;
  const failureObserver = includeFailureObservation ? ", modellang_failure_observer" : "";
  const lines = [
    "-- Generated least-privilege application boundary.",
    `REVOKE CREATE ON SCHEMA ${quoteIdent(schema)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${failureObserver};`,
    `REVOKE ALL ON SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${failureObserver};`,
  ...
  for (const entity of ir.entities) {
    ...
    lines.push(
      `REVOKE ALL ON TABLE ${table} FROM PUBLIC, modellang_app, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${failureObserver};`,
    );
  }
  lines.push("");
  ...
  for (const consumer of ir.consumers) {
    lines.push(
      `REVOKE ALL ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_recovery, modellang_publication_recovery${failureObserver};`,
      `GRANT EXECUTE ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) TO modellang_consumer;`,
    );
  }
  lines.push(
    `REVOKE ALL ON ALL TABLES IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${failureObserver};`,
    `REVOKE ALL ON ALL FUNCTIONS IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${failureObserver};`,
...
END
$modellang_upgrade$;
${generateFailureObserverInfrastructureStatements(ir).join("\n")}
RESET ROLE;
...
export function generatePostgres(ir: ModelIR, plan: DecisionPlan = generateDecisionPlan(ir)): PostgresOutput {
  return {
    ...
    "004_grants.sql": generateGrants(ir, true, true),
