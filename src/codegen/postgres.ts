...
function failureObserverRoleCheck(): string[] {
  return [
    "  IF NOT EXISTS (",
    "    SELECT 1 FROM pg_catalog.pg_auth_members AS membership",
...
function failureAcknowledgerRoleCheck(): string[] {
  return [
    "  IF NOT EXISTS (",
    "    SELECT 1 FROM pg_catalog.pg_auth_members AS membership",
...
function failureClaimantRoleCheck(): string[] {
  return [
    "  IF NOT EXISTS (",
    "    SELECT 1 FROM pg_catalog.pg_auth_members AS membership",
...
export function generateFailureAcknowledgementInfrastructureStatements(ir: ModelIR): string[] {
  const internal = ir.model.naming.internalSchema;
  const outbox = qname(internal, "event_outbox");
  const failures = qname(internal, "consumer_failure");
  const publicationAudit = qname(internal, "publication_failure_acknowledgement");
  const consumerAudit = qname(internal, "consumer_failure_acknowledgement");
  const consumerIds = ir.consumers.map((consumer) => `'${consumer.id.replaceAll("'", "''")}'`).join(", ");
  const invalidConsumerIdentity = consumerIds.length > 0 ? `p_consumer_id NOT IN (${consumerIds})` : "TRUE";
  return [
    `CREATE TABLE IF NOT EXISTS ${publicationAudit} (`,
    `  ${quoteIdent("id")} bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,`,
    `  ${quoteIdent("event_outbox_id")} uuid NOT NULL REFERENCES ${outbox} (${quoteIdent("id")}),`,
    `  ${quoteIdent("event_id")} text NOT NULL,`,
    `  ${quoteIdent("recovery_generation")} integer NOT NULL,`,
    ...
    `  ${quoteIdent("database_principal")} name NOT NULL,`,
    `  ${quoteIdent("occurred_at")} timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),`,
    `  CONSTRAINT ${quoteIdent("uq_publication_failure_acknowledgement_cycle")} UNIQUE (${quoteIdent("event_outbox_id")}, ${quoteIdent("recovery_generation")}),`,
    `  CONSTRAINT ${quoteIdent("ck_publication_failure_acknowledgement")} CHECK (${quoteIdent("recovery_generation")} >= 0 AND ${quoteIdent("reason_code")} ~ '^[A-Z][A-Z0-9_]{0,63}$')`,
    ");",
    ...
    `  ${quoteIdent("database_principal")} name NOT NULL,`,
    `  ${quoteIdent("occurred_at")} timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),`,
    `  CONSTRAINT ${quoteIdent("uq_consumer_failure_acknowledgement_cycle")} UNIQUE (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}, ${quoteIdent("recovery_generation")}),`,
    `  CONSTRAINT ${quoteIdent("fk_consumer_failure_acknowledgement")} FOREIGN KEY (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}) REFERENCES ${failures} (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}),`,
    `  CONSTRAINT ${quoteIdent("ck_consumer_failure_acknowledgement")} CHECK (${quoteIdent("recovery_generation")} >= 0 AND ${quoteIdent("reason_code")} ~ '^[A-Z][A-Z0-9_]{0,63}$')`,
    ");",
    `CREATE OR REPLACE FUNCTION ${qname(internal, "acknowledge_terminal_publication_failure")}(p_event_id uuid, p_reason_code text) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_stable_event_id text;",
    "  v_disposition text;",
    "  v_recovery_generation integer;",
    "BEGIN",
    ...failureAcknowledgerRoleCheck(),
    "  IF p_event_id IS NULL OR p_reason_code IS NULL OR p_reason_code !~ '^[A-Z][A-Z0-9_]{0,63}$' THEN",
    "    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'ML_PUBLICATION_FAILURE_ACKNOWLEDGEMENT';",
    "  END IF;",
    `  SELECT ${quoteIdent("event_id")}, ${quoteIdent("publication_disposition")}, ${quoteIdent("publication_recovery_generation")}`,
    ...
    "  END IF;",
    `  IF EXISTS (SELECT 1 FROM ${publicationAudit} WHERE ${quoteIdent("event_outbox_id")} = p_event_id AND ${quoteIdent("recovery_generation")} = v_recovery_generation) THEN`,
    ...
    "  END IF;",
    `  INSERT INTO ${publicationAudit} (${quoteIdent("event_outbox_id")}, ${quoteIdent("event_id")}, ${quoteIdent("recovery_generation")}, ${quoteIdent("reason_code")}, ${quoteIdent("database_principal")})`,
    ...
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "acknowledge_terminal_publication_failure")}(uuid, text) FROM PUBLIC;`,
    `CREATE OR REPLACE FUNCTION ${qname(internal, "acknowledge_terminal_consumer_failure")}(p_consumer_id text, p_event_id text, p_reason_code text) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_disposition text;",
    "  v_recovery_generation integer;",
    ...
    "  p_event_id := p_event_id::uuid::text;",
    "  PERFORM pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(p_consumer_id || ':' || p_event_id, 0));",
    `  SELECT ${quoteIdent("disposition")}, ${quoteIdent("recovery_generation")} INTO v_disposition, v_recovery_generation`,
    `  FROM ${failures} WHERE ${quoteIdent("consumer_id")} = p_consumer_id AND ${quoteIdent("source_event_id")} = p_event_id FOR UPDATE;`,
    "  IF NOT FOUND OR v_disposition <> 'deadLetter' THEN",
    "    RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'ML_CONSUMER_FAILURE_ACKNOWLEDGEMENT_STATE';",
    "  END IF;",
    `  IF EXISTS (SELECT 1 FROM ${consumerAudit} WHERE ${quoteIdent("consumer_id")} = p_consumer_id AND ${quoteIdent("source_event_id")} = p_event_id AND ${quoteIdent("recovery_generation")} = v_recovery_generation) THEN`,
    "    RETURN pg_catalog.jsonb_build_object('status', 'alreadyAcknowledged', 'acknowledged', TRUE, 'recoveryGeneration', v_recovery_generation);",
    "  END IF;",
    `  INSERT INTO ${consumerAudit} (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}, ${quoteIdent("recovery_generation")}, ${quoteIdent("reason_code")}, ${quoteIdent("database_principal")})`,
    "  VALUES (p_consumer_id, p_event_id, v_recovery_generation, p_reason_code, session_user);",
    "  RETURN pg_catalog.jsonb_build_object('status', 'acknowledged', 'acknowledged', TRUE, 'recoveryGeneration', v_recovery_generation);",
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "acknowledge_terminal_consumer_failure")}(text, text, text) FROM PUBLIC;`,
  ];
}

export function generateFailureClaimInfrastructureStatements(ir: ModelIR): string[] {
  const internal = ir.model.naming.internalSchema;
  const outbox = qname(internal, "event_outbox");
  const failures = qname(internal, "consumer_failure");
  const publicationClaims = qname(internal, "publication_failure_claim");
  const consumerClaims = qname(internal, "consumer_failure_claim");
  const consumerIds = ir.consumers.map((consumer) => `'${consumer.id.replaceAll("'", "''")}'`).join(", ");
  const invalidConsumerIdentity = consumerIds.length > 0 ? `p_consumer_id NOT IN (${consumerIds})` : "TRUE";
  return [
    `CREATE TABLE IF NOT EXISTS ${publicationClaims} (`,
    `  ${quoteIdent("id")} bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,`,
    `  ${quoteIdent("event_outbox_id")} uuid NOT NULL REFERENCES ${outbox} (${quoteIdent("id")}),`,
    `  ${quoteIdent("event_id")} text NOT NULL,`,
    `  ${quoteIdent("recovery_generation")} integer NOT NULL,`,
    `  ${quoteIdent("claimant_principal")} name NOT NULL,`,
    `  ${quoteIdent("claimed_at")} timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),`,
    `  CONSTRAINT ${quoteIdent("uq_publication_failure_claim_cycle")} UNIQUE (${quoteIdent("event_outbox_id")}, ${quoteIdent("recovery_generation")}),`,
    `  CONSTRAINT ${quoteIdent("ck_publication_failure_claim")} CHECK (${quoteIdent("recovery_generation")} >= 0)`,
    ");",
    `CREATE TABLE IF NOT EXISTS ${consumerClaims} (`,
    `  ${quoteIdent("id")} bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,`,
    `  ${quoteIdent("consumer_id")} text NOT NULL,`,
    `  ${quoteIdent("source_event_id")} text NOT NULL,`,
    `  ${quoteIdent("recovery_generation")} integer NOT NULL,`,
    `  ${quoteIdent("claimant_principal")} name NOT NULL,`,
    `  ${quoteIdent("claimed_at")} timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),`,
    `  CONSTRAINT ${quoteIdent("uq_consumer_failure_claim_cycle")} UNIQUE (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}, ${quoteIdent("recovery_generation")}),`,
    `  CONSTRAINT ${quoteIdent("fk_consumer_failure_claim")} FOREIGN KEY (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}) REFERENCES ${failures} (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}),`,
    `  CONSTRAINT ${quoteIdent("ck_consumer_failure_claim")} CHECK (${quoteIdent("recovery_generation")} >= 0)`,
    ");",
    `CREATE OR REPLACE FUNCTION ${qname(internal, "claim_terminal_publication_failure")}(p_event_id uuid) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_stable_event_id text;",
    "  v_disposition text;",
    ...
    "  END IF;",
    `  IF EXISTS (SELECT 1 FROM ${publicationClaims} WHERE ${quoteIdent("event_outbox_id")} = p_event_id AND ${quoteIdent("recovery_generation")} = v_recovery_generation) THEN`,
    "    RETURN pg_catalog.jsonb_build_object('status', 'alreadyClaimed', 'claimed', TRUE, 'recoveryGeneration', v_recovery_generation);",
    "  END IF;",
    `  INSERT INTO ${publicationClaims} (${quoteIdent("event_outbox_id")}, ${quoteIdent("event_id")}, ${quoteIdent("recovery_generation")}, ${quoteIdent("claimant_principal")})`,
    ...
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "claim_terminal_publication_failure")}(uuid) FROM PUBLIC;`,
    `CREATE OR REPLACE FUNCTION ${qname(internal, "claim_terminal_consumer_failure")}(p_consumer_id text, p_event_id text) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_disposition text;",
    "  v_recovery_generation integer;",
    ...
    "  END IF;",
    `  IF EXISTS (SELECT 1 FROM ${consumerClaims} WHERE ${quoteIdent("consumer_id")} = p_consumer_id AND ${quoteIdent("source_event_id")} = p_event_id AND ${quoteIdent("recovery_generation")} = v_recovery_generation) THEN`,
    "    RETURN pg_catalog.jsonb_build_object('status', 'alreadyClaimed', 'claimed', TRUE, 'recoveryGeneration', v_recovery_generation);",
    "  END IF;",
    `  INSERT INTO ${consumerClaims} (${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}, ${quoteIdent("recovery_generation")}, ${quoteIdent("claimant_principal")})`,
    "  VALUES (p_consumer_id, p_event_id, v_recovery_generation, session_user);",
    "  RETURN pg_catalog.jsonb_build_object('status', 'claimed', 'claimed', TRUE, 'recoveryGeneration', v_recovery_generation);",
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "claim_terminal_consumer_failure")}(text, text) FROM PUBLIC;`,
  ];
}
...
export function generateFailureObserverInfrastructureStatements(
  ...
  const outbox = qname(internal, "event_outbox");
  const failures = qname(internal, "consumer_failure");
  const audit = qname(internal, "failure_observation_audit");
  const recoverableConsumerIds = ir.consumers
    .filter((consumer) => consumer.failurePolicy.mode === "deadLetterAfterMaxAttempts" && consumer.failurePolicy.recovery === "manual")
    .map((consumer) => `'${consumer.id.replaceAll("'", "''")}'`);
  const consumerRecoveryEligibility = recoverableConsumerIds.length > 0
    ...
    : "FALSE";
  const publicationAcknowledgedSelect = `, EXISTS (SELECT 1 FROM ${qname(internal, "publication_failure_acknowledgement")} AS acknowledgement WHERE acknowledgement.${quoteIdent("event_outbox_id")} = row_value.${quoteIdent("id")} AND acknowledgement.${quoteIdent("recovery_generation")} = row_value.${quoteIdent("publication_recovery_generation")}) AS acknowledged`;
  const publicationAcknowledgedProjection = ", 'acknowledged', acknowledged";
  const consumerAcknowledgedSelect = `, EXISTS (SELECT 1 FROM ${qname(internal, "consumer_failure_acknowledgement")} AS acknowledgement WHERE acknowledgement.${quoteIdent("consumer_id")} = row_value.${quoteIdent("consumer_id")} AND acknowledgement.${quoteIdent("source_event_id")} = row_value.${quoteIdent("source_event_id")} AND acknowledgement.${quoteIdent("recovery_generation")} = row_value.${quoteIdent("recovery_generation")}) AS acknowledged`;
  const consumerAcknowledgedProjection = ", 'acknowledged', acknowledged";
  const publicationClaimedSelect = `, EXISTS (SELECT 1 FROM ${qname(internal, "publication_failure_claim")} AS failure_claim WHERE failure_claim.${quoteIdent("event_outbox_id")} = row_value.${quoteIdent("id")} AND failure_claim.${quoteIdent("recovery_generation")} = row_value.${quoteIdent("publication_recovery_generation")}) AS claimed`;
  const publicationClaimedProjection = ", 'claimed', claimed";
  const consumerClaimedSelect = `, EXISTS (SELECT 1 FROM ${qname(internal, "consumer_failure_claim")} AS failure_claim WHERE failure_claim.${quoteIdent("consumer_id")} = row_value.${quoteIdent("consumer_id")} AND failure_claim.${quoteIdent("source_event_id")} = row_value.${quoteIdent("source_event_id")} AND failure_claim.${quoteIdent("recovery_generation")} = row_value.${quoteIdent("recovery_generation")}) AS claimed`;
  const consumerClaimedProjection = ", 'claimed', claimed";
  return [
    `CREATE TABLE IF NOT EXISTS ${audit} (`,
    `  ${quoteIdent("id")} bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,`,
    `  ${quoteIdent("database_principal")} name NOT NULL,`,
    `  ${quoteIdent("failure_kind")} text NOT NULL,`,
    `  ${quoteIdent("snapshot_at")} timestamptz NOT NULL,`,
    `  ${quoteIdent("after_terminal_at")} timestamptz,`,
    `  ${quoteIdent("after_identity")} text,`,
    `  ${quoteIdent("requested_limit")} integer NOT NULL,`,
    `  ${quoteIdent("returned_count")} integer NOT NULL,`,
    `  ${quoteIdent("has_more")} boolean NOT NULL,`,
    `  ${quoteIdent("occurred_at")} timestamptz NOT NULL DEFAULT pg_catalog.clock_timestamp(),`,
    `  CONSTRAINT ${quoteIdent("ck_failure_observation_kind")} CHECK (${quoteIdent("failure_kind")} IN ('publication', 'consumer')),`,
    `  CONSTRAINT ${quoteIdent("ck_failure_observation_cursor")} CHECK ((${quoteIdent("after_terminal_at")} IS NULL) = (${quoteIdent("after_identity")} IS NULL)),`,
    `  CONSTRAINT ${quoteIdent("ck_failure_observation_counts")} CHECK (${quoteIdent("requested_limit")} BETWEEN 1 AND 100 AND ${quoteIdent("returned_count")} BETWEEN 0 AND ${quoteIdent("requested_limit")})`,
    ");",
    `CREATE INDEX IF NOT EXISTS ${quoteIdent("ix_event_outbox_terminal_observation")} ON ${outbox} (${quoteIdent("publication_terminal_at")}, ${quoteIdent("id")}) WHERE ${quoteIdent("publication_disposition")} = 'deadLetter';`,
    `CREATE INDEX IF NOT EXISTS ${quoteIdent("ix_consumer_failure_terminal_observation")} ON ${failures} (${quoteIdent("terminal_at")}, ${quoteIdent("consumer_id")}, ${quoteIdent("source_event_id")}) WHERE ${quoteIdent("disposition")} = 'deadLetter';`,
    `CREATE OR REPLACE FUNCTION ${qname(internal, "observe_terminal_publications")}(p_snapshot_at timestamptz, p_after_terminal_at timestamptz, p_after_event_id uuid, p_limit integer) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_snapshot_at timestamptz;",
    "  v_result jsonb;",
    "BEGIN",
    ...failureObserverRoleCheck(),
    ...
    "  END IF;",
    "  WITH candidates AS (",
    `    SELECT row_value.${quoteIdent("publication_terminal_at")} AS terminal_at, row_value.${quoteIdent("id")} AS event_instance_id,`,
    `      row_value.${quoteIdent("event_id")} AS event_id, row_value.${quoteIdent("publication_failure_count")} AS failure_count,`,
    `      row_value.${quoteIdent("publication_total_failure_count")} AS total_failure_count, row_value.${quoteIdent("publication_max_attempts")} AS max_attempts,`,
    `      row_value.${quoteIdent("last_publication_error_code")} AS last_error_code, row_value.${quoteIdent("publication_recovery_generation")} AS recovery_generation,`,
    `      row_value.${quoteIdent("publication_recovery_mode")} = 'manual' AS recovery_eligible${publicationAcknowledgedSelect}${publicationClaimedSelect}`,
    `    FROM ${outbox} AS row_value`,
    `    WHERE row_value.${quoteIdent("publication_disposition")} = 'deadLetter' AND row_value.${quoteIdent("publication_terminal_at")} <= v_snapshot_at`,
    `      AND (p_after_terminal_at IS NULL OR (row_value.${quoteIdent("publication_terminal_at")}, row_value.${quoteIdent("id")}) > (p_after_terminal_at, p_after_event_id))`,
    `    ORDER BY row_value.${quoteIdent("publication_terminal_at")}, row_value.${quoteIdent("id")} LIMIT p_limit + 1`,
    ...
    `      'recoveryEligible', recovery_eligible${publicationAcknowledgedProjection}${publicationClaimedProjection}) ORDER BY terminal_at, event_instance_id) FROM page_rows), '[]'::jsonb),`,
    ...
    `  INSERT INTO ${audit} (${quoteIdent("database_principal")}, ${quoteIdent("failure_kind")}, ${quoteIdent("snapshot_at")}, ${quoteIdent("after_terminal_at")}, ${quoteIdent("after_identity")}, ${quoteIdent("requested_limit")}, ${quoteIdent("returned_count")}, ${quoteIdent("has_more")})`,
    ...
    "  RETURN v_result;",
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "observe_terminal_publications")}(timestamptz, timestamptz, uuid, integer) FROM PUBLIC;`,
    `CREATE OR REPLACE FUNCTION ${qname(internal, "observe_terminal_consumers")}(p_snapshot_at timestamptz, p_after_terminal_at timestamptz, p_after_consumer_id text, p_after_event_id uuid, p_limit integer) RETURNS jsonb`,
    "LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$",
    "DECLARE",
    "  v_snapshot_at timestamptz;",
    "  v_result jsonb;",
    ...
    "  END IF;",
    "  WITH candidates AS (",
    `    SELECT row_value.${quoteIdent("terminal_at")} AS terminal_at, row_value.${quoteIdent("consumer_id")} AS consumer_id,`,
    `      row_value.${quoteIdent("source_event_id")}::uuid AS event_instance_id, row_value.${quoteIdent("failure_count")} AS failure_count,`,
    `      row_value.${quoteIdent("total_failure_count")} AS total_failure_count, row_value.${quoteIdent("max_attempts")} AS max_attempts,`,
    `      row_value.${quoteIdent("last_error_code")} AS last_error_code, row_value.${quoteIdent("recovery_generation")} AS recovery_generation,`,
    `      ${consumerRecoveryEligibility} AS recovery_eligible${consumerAcknowledgedSelect}${consumerClaimedSelect}`,
    `    FROM ${failures} AS row_value`,
    `    WHERE row_value.${quoteIdent("disposition")} = 'deadLetter' AND row_value.${quoteIdent("terminal_at")} <= v_snapshot_at`,
    `      AND (p_after_terminal_at IS NULL OR (row_value.${quoteIdent("terminal_at")}, row_value.${quoteIdent("consumer_id")}, row_value.${quoteIdent("source_event_id")}::uuid) > (p_after_terminal_at, p_after_consumer_id, p_after_event_id))`,
    `    ORDER BY row_value.${quoteIdent("terminal_at")}, row_value.${quoteIdent("consumer_id")}, row_value.${quoteIdent("source_event_id")}::uuid LIMIT p_limit + 1`,
    ...
    "      'failureCount', failure_count, 'totalFailureCount', total_failure_count, 'maxAttempts', max_attempts,",
    "      'lastErrorCode', last_error_code, 'terminalAt', terminal_at, 'recoveryGeneration', recovery_generation,",
    `      'recoveryEligible', recovery_eligible${consumerAcknowledgedProjection}${consumerClaimedProjection}) ORDER BY terminal_at, consumer_id, event_instance_id) FROM page_rows), '[]'::jsonb),`,
    "    'nextCursor', CASE WHEN stats.candidate_count > p_limit THEN (SELECT pg_catalog.jsonb_build_object(",
    ...
    "  RETURN v_result;",
    "END $modellang$;",
    `REVOKE ALL ON FUNCTION ${qname(internal, "observe_terminal_consumers")}(timestamptz, timestamptz, text, uuid, integer) FROM PUBLIC;`,
  ];
}
...
function generateSchema(ir: ModelIR): string {
  ...
  lines.push(
    ...
    ...generateFailureAcknowledgementInfrastructureStatements(ir),
...
function projectionJson(
  ...
  const values = projection.fields.flatMap((selected, index) => {
    ...
    const column = `${record}.${quoteIdent(field.naming.sqlColumn)}`;
    const currentFieldPath = [...projectionFieldPath, selected.id];
    let value: string;
    if (!selected.nestedProjectionId) {
      value = fieldJson(field, column);
    } else {
      ...
      const nestedPath = [...aliasPath, index];
      const alias = `v_projection_${nestedPath.join("_")}`;
      const lookup = `(SELECT ${projectionJson(ir, nestedProjection.id, quoteIdent(alias), context, currentFieldPath, nestedPath)} FROM ${qname(ir.model.naming.sqlSchema, nestedEntity.naming.sqlTable)} AS ${quoteIdent(alias)} WHERE ${quoteIdent(alias)}.${quoteIdent(nestedId.naming.sqlColumn)} = ${column})`;
      value = field.optional ? `CASE WHEN ${column} IS NULL THEN NULL ELSE ${lookup} END` : lookup;
    }
    if (selected.redactable) {
      const rule = context?.query?.disclosures?.find((candidate) =>
        candidate.projectionFieldPath.length === currentFieldPath.length
        && candidate.projectionFieldPath.every((fieldIdValue, pathIndex) => fieldIdValue === currentFieldPath[pathIndex]));
      value = rule ? `CASE WHEN ((${lowerExpression(rule.expression, context!)}) IS TRUE) THEN ${value} ELSE NULL END` : "NULL";
    }
...
function generateQuery(ir: ModelIR, query: IRQuery): string {
  ...
  if (query.pagination) {
    ...
    body.push(
      ...
      `    SELECT ${projectionJson(ir, query.returnProjectionId, "v_row", context)} AS ${quoteIdent("item")},`,
...
function generateGrants(ir: ModelIR): string {
  const schema = ir.model.naming.sqlSchema;
  const internal = ir.model.naming.internalSchema;
  const privateOperations = ", modellang_failure_observer, modellang_failure_acknowledger, modellang_failure_claimant";
  const lines = [
    "-- Generated least-privilege application boundary.",
    `REVOKE CREATE ON SCHEMA ${quoteIdent(schema)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${privateOperations};`,
    `REVOKE ALL ON SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${privateOperations};`,
    ...
    `GRANT USAGE ON SCHEMA ${quoteIdent(internal)} TO modellang_failure_observer;`,
    `GRANT USAGE ON SCHEMA ${quoteIdent(internal)} TO modellang_failure_acknowledger;`,
    `GRANT USAGE ON SCHEMA ${quoteIdent(internal)} TO modellang_failure_claimant;`,
  ];
  lines.push("");
  for (const entity of ir.entities) {
    const table = qname(schema, entity.naming.sqlTable);
    lines.push(
      `REVOKE ALL ON TABLE ${table} FROM PUBLIC, modellang_app, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${privateOperations};`,
    );
  }
  lines.push("");
  ...
  for (const consumer of ir.consumers) {
    lines.push(
      `REVOKE ALL ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_recovery, modellang_publication_recovery${privateOperations};`,
      `GRANT EXECUTE ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) TO modellang_consumer;`,
    );
  }
  lines.push(
    `REVOKE ALL ON ALL TABLES IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${privateOperations};`,
    `REVOKE ALL ON ALL FUNCTIONS IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer, modellang_recovery, modellang_publication_recovery${privateOperations};`,
