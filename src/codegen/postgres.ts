...
export function generateEventOutboxInfrastructureStatements(ir: ModelIR): string[] {
  ...
  return [
    ...
    "BEGIN",
    ...dispatcherCheck,
    `  UPDATE ${outbox} SET ${quoteIdent("published_at")} = pg_catalog.clock_timestamp(), ${quoteIdent("lease_token")} = (NULL::uuid), ${quoteIdent("leased_until")} = (NULL::timestamptz)`,
    `  WHERE ${quoteIdent("id")} = p_event_id AND ${quoteIdent("published_at")} IS NULL AND ${quoteIdent("lease_token")} = p_lease_token AND ${quoteIdent("leased_until")} > pg_catalog.clock_timestamp();`,
    ...
    `  WHERE ${quoteIdent("id")} = p_event_id AND ${quoteIdent("published_at")} IS NULL AND ${quoteIdent("lease_token")} = p_lease_token;`,
    "  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'ML_OUTBOX_LEASE'; END IF;",
    "END $modellang$;",
...
function generateAction(ir: ModelIR, action: IRAction, decision: ActionDecisionPlan): string {
  ...
  action.emittedEventIds.forEach((emittedEventId, ordinal) => {
    ...
    body.push(
      `  INSERT INTO ${qname(internal, "event_outbox")} (${quoteIdent("model_id")}, ${quoteIdent("model_version")}, ${quoteIdent("source_hash")}, ${quoteIdent("event_id")}, ${quoteIdent("event_name")}, ${quoteIdent("payload_entity_id")}, ${quoteIdent("action_id")}, ${quoteIdent("principal_id")}, ${quoteIdent("target_id")}, ${quoteIdent("payload")}, ${quoteIdent("correlation_id")}, ${quoteIdent("causation_id")}, ${quoteIdent("action_audit_id")}, ${quoteIdent("command_receipt_id")}, ${quoteIdent("ordinal")})`,
      `  VALUES ('${ir.model.id.replaceAll("'", "''")}', '${ir.model.version.replaceAll("'", "''")}', '${ir.model.sourceHash.replaceAll("'", "''")}', '${event.id.replaceAll("'", "''")}', '${event.name.replaceAll("'", "''")}', '${event.payloadEntityId.replaceAll("'", "''")}', '${action.id.replaceAll("'", "''")}', v_principal_id, v_result.${quoteIdent("id")}, v_response, v_correlation_id, v_causation_id, v_action_audit_id, v_receipt_id, ${ordinal});`,
      "",
    );
  });
  if (action.idempotency) {
...
function generateGrants(ir: ModelIR, includeApplicability = true): string {
  ...
  const lines = [
    ...
    `REVOKE CREATE ON SCHEMA ${quoteIdent(schema)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer;`,
    `REVOKE ALL ON SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer;`,
  ...
  for (const entity of ir.entities) {
    ...
    lines.push(
      `REVOKE ALL ON TABLE ${table} FROM PUBLIC, modellang_app, modellang_dispatcher, modellang_consumer;`,
    );
  }
  lines.push("");
  ...
  for (const consumer of ir.consumers) {
    lines.push(
      `REVOKE ALL ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher;`,
      `GRANT EXECUTE ON FUNCTION ${qname(internal, consumer.naming.sqlFunction)}(jsonb) TO modellang_consumer;`,
    );
  }
  lines.push(
    `REVOKE ALL ON ALL TABLES IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer;`,
    `REVOKE ALL ON ALL FUNCTIONS IN SCHEMA ${quoteIdent(internal)} FROM PUBLIC, modellang_app, modellang_gateway, modellang_dispatcher, modellang_consumer;`,
