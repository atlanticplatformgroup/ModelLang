...
export function generateFailureObserverInfrastructureStatements(ir: ModelIR, includeAcknowledgement = true): string[] {
  const internal = ir.model.naming.internalSchema;
  const outbox = qname(internal, "event_outbox");
  const failures = qname(internal, "consumer_failure");
  ...
  return [
    ...
    `      row_value.${quoteIdent("publication_recovery_mode")} = 'manual' AS recovery_eligible${publicationAcknowledgedSelect}`,
    ...
    `      'recoveryEligible', recovery_eligible${publicationAcknowledgedProjection}) ORDER BY terminal_at, event_instance_id) FROM page_rows), '[]'::jsonb),`,
    ...
    `      ${consumerRecoveryEligibility} AS recovery_eligible${consumerAcknowledgedSelect}`,
    ...
    "      'failureCount', failure_count, 'totalFailureCount', total_failure_count, 'maxAttempts', max_attempts,",
    "      'lastErrorCode', last_error_code, 'terminalAt', terminal_at, 'recoveryGeneration', recovery_generation,",
    `      'recoveryEligible', recovery_eligible${consumerAcknowledgedProjection}) ORDER BY terminal_at, consumer_id, event_instance_id) FROM page_rows), '[]'::jsonb),`,
    "    'nextCursor', CASE WHEN stats.candidate_count > p_limit THEN (SELECT pg_catalog.jsonb_build_object(",
...
function generateGrants(ir: ModelIR, includeApplicability = true, includeFailureObservation = false, includeFailureAcknowledgement = false): string {
  const schema = ir.model.naming.sqlSchema;
  const internal = ir.model.naming.internalSchema;
  ...
  const privateOperations = `${failureObserver}${failureAcknowledger}`;
  const lines = [
...
END
$modellang_upgrade$;
${generateFailureObserverInfrastructureStatements(ir, false).join("\n")}
RESET ROLE;
...
${generateFailureObserverInfrastructureStatements(ir, true).join("\n")}
RESET ROLE;
...
export function generatePostgres(ir: ModelIR, plan: DecisionPlan = generateDecisionPlan(ir)): PostgresOutput {
  return {
    ...
    "004_grants.sql": generateGrants(ir, true, true, true),
