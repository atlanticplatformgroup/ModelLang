...
export async function recoverIndexReservation(client: ConsumerRecoveryDatabaseClient, eventId: string, reasonCode: string): Promise<ConsumerRecoveryOutcome> {
  const response = await client.query<{ result: ConsumerRecoveryOutcome }>("SELECT \"model_reservations_internal\".\"recover_consumer_failure\"($1, $2, $3) AS result", ["consumer:con_20d694c9a0a274dc79c6168e47d25968", eventId, reasonCode]);
