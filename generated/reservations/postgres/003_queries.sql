...
BEGIN
  ...
  IF NOT ((TRUE) IS TRUE) THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'ML_AUTHORIZATION:authorize:reservationsForResource';
  END IF;
  ...
  FROM (
    SELECT jsonb_build_object('id', v_row."id", 'resource', v_row."resource_id", 'reservedBy', v_row."reserved_by_id", 'startsAt', v_row."starts_at", 'endsAt', v_row."ends_at") AS "item",
