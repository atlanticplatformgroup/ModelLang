...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR v_version IS DISTINCT FROM '0.27.0'
     OR v_source_hash IS DISTINCT FROM 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90' THEN
    RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'ML_MIGRATION_BASELINE:sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90';
  END IF;
END
$modellang_upgrade$;
...
BEGIN
  ...
  VALUES ('model:Procurement', '0.27.0', 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90', 'action:act_1e35db0451b1461e941af6283d86dca2', v_principal_id, v_idempotency_key, v_request_hash, v_correlation_id, v_causation_id)
  ...
  IF v_receipt_id IS NULL THEN
    ...
    IF v_receipt_source_hash IS DISTINCT FROM 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90' OR v_receipt_request_hash IS DISTINCT FROM v_request_hash THEN
...
;
  ...
  v_response := jsonb_build_object('id', v_result."id", 'createdAt', v_result."created_at", 'requester', v_result."requester_id", 'amount', jsonb_build_object('currency', 'USD', 'amount', (v_result."amount"::numeric(20, 2))::text), 'status', v_result."status", 'approvedBy', v_result."approved_by_id", 'approvedByRoles', v_result."approved_by_roles", 'approvalObserved', v_result."approval_observed");
  INSERT INTO "model_procurement_internal"."action_audit" ("action_id", "database_principal", "principal_id", "target_id", "identity_issuer", "identity_subject", "model_id", "model_version", "source_hash", "authorization_rule_id", "decision_outcome", "policy_id", "authority_id", "decision_evidence", "correlation_id", "causation_id", "command_receipt_id")
  VALUES ('action:act_ed2374e822704c51a2925338253d05d2', session_user, v_principal_id, v_result."id", v_identity_issuer, v_identity_subject, 'model:Procurement', '0.27.0', 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90', 'authorize:action:act_ed2374e822704c51a2925338253d05d2', 'executed', v_authority_policy_id, v_authority_id, pg_catalog.jsonb_build_object('version', 2, 'outcome', 'executed', 'model', pg_catalog.jsonb_build_object('id', 'model:Procurement', 'version', '0.27.0', 'sourceHash', 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90'), 'actionId', 'action:act_ed2374e822704c51a2925338253d05d2', 'command', pg_catalog.jsonb_build_object('correlationId', v_correlation_id, 'causationId', v_causation_id, 'receiptId', v_receipt_id), 'authorization', pg_catalog.jsonb_build_object('ruleId', 'authorize:action:act_ed2374e822704c51a2925338253d05d2', 'outcome', 'passed', 'policyId', v_authority_policy_id, 'authorityId', v_authority_id), 'requirements', pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object('ruleId', 'require:action:act_ed2374e822704c51a2925338253d05d2.is_draft', 'outcome', 'passed', 'policyIds', pg_catalog.jsonb_build_array()))), v_correlation_id, v_causation_id, v_receipt_id)
  RETURNING "id" INTO v_action_audit_id;
...
BEGIN
  ...
  IF p_envelope->>'eventId' IS DISTINCT FROM 'event:evt_30d694c9a0a274dc79c6168e47d25968'
     ...
     OR v_source_model_version IS DISTINCT FROM '0.27.0'
     OR v_source_hash IS DISTINCT FROM 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90'
  ...
  VALUES ('model:Procurement', '0.27.0', 'sha256:84969af54e131f4e9751a5798913868b43f2b70f3345402a4ad03a9b5d781c90', 'event:evt_50d694c9a0a274dc79c6168e47d25968', 'ApprovalObserved', 'entity:ent_9bc680209327484c8e98f5f740bcc702', 'consumer:con_10d694c9a0a274dc79c6168e47d25968', v_result."id", v_response, v_correlation_id, v_source_event_id::text, v_consumer_audit_id, 0, 5, 'manual');
