...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object('id', v_row."id", 'requester', v_row."requester_id", 'amount', v_row."amount"::text, 'status', v_row."status", 'approvedBy', v_row."approved_by_id", 'approvedByRoles', v_row."approved_by_roles") AS "item",
