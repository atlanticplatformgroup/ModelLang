...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR v_version IS DISTINCT FROM '0.38.0'
     OR v_source_hash IS DISTINCT FROM 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77' THEN
    RAISE EXCEPTION USING ERRCODE = '55000', MESSAGE = 'ML_MIGRATION_BASELINE:sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77';
  END IF;
END
$modellang_upgrade$;
...
ALTER TABLE "model_procurement_internal"."event_outbox" ADD COLUMN IF NOT EXISTS "publication_total_failure_count" integer NOT NULL DEFAULT 0;
ALTER TABLE "model_procurement_internal"."event_outbox" ADD COLUMN IF NOT EXISTS "publication_max_attempts" integer;
ALTER TABLE "model_procurement_internal"."event_outbox" ADD COLUMN IF NOT EXISTS "publication_recovery_mode" text NOT NULL DEFAULT 'none';
ALTER TABLE "model_procurement_internal"."event_outbox" ADD COLUMN IF NOT EXISTS "publication_recovery_generation" integer NOT NULL DEFAULT 0;
...
ALTER TABLE "model_procurement_internal"."event_outbox" ADD COLUMN IF NOT EXISTS "last_publication_recovered_at" timestamptz;
UPDATE "model_procurement_internal"."event_outbox" SET "publication_total_failure_count" = "publication_failure_count" WHERE "publication_total_failure_count" < "publication_failure_count";
...
ALTER TABLE "model_procurement_internal"."event_outbox" ADD CONSTRAINT "ck_event_outbox_delivery" CHECK ("delivery_attempts" >= 0 AND "publication_failure_count" >= 0 AND "publication_total_failure_count" >= "publication_failure_count" AND "publication_recovery_generation" >= 0 AND ("publication_max_attempts" IS NULL OR "publication_max_attempts" BETWEEN 1 AND 1000) AND ("publication_recovery_mode" = 'none' OR ("publication_recovery_mode" = 'manual' AND "publication_max_attempts" IS NOT NULL)) AND (("lease_token" IS NULL) = ("leased_until" IS NULL)));
...
END
$modellang$;
CREATE TABLE IF NOT EXISTS "model_procurement_internal"."publication_recovery_audit" (
  "id" bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
BEGIN
  ...
  IF p_error_code IS NULL OR p_error_code !~ '^ML_[A-Z_]+$' OR pg_catalog.length(p_error_code) > 64 THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'ML_VALIDATION:boundary:event_outbox';
  END IF;
  UPDATE "model_procurement_internal"."event_outbox" SET "publication_failure_count" = "publication_failure_count" + 1, "publication_total_failure_count" = "publication_total_failure_count" + 1,
...
CREATE OR REPLACE FUNCTION "model_procurement_internal"."recover_event_publication"(p_event_id uuid, p_reason_code text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $modellang$
DECLARE
...
BEGIN
  ...
  END IF;
  v_recovery_generation := v_recovery_generation + 1;
  UPDATE "model_procurement_internal"."event_outbox" SET "publication_failure_count" = 0, "publication_disposition" = 'pending',
  ...
  WHERE "id" = p_event_id;
  INSERT INTO "model_procurement_internal"."publication_recovery_audit" ("event_outbox_id", "event_id", "recovery_generation", "prior_failure_count", "total_failure_count", "prior_error_code", "reason_code", "database_principal")
...
END $modellang$;
REVOKE ALL ON FUNCTION "model_procurement_internal"."recover_event_publication"(uuid, text) FROM PUBLIC;
RESET ROLE;
...
BEGIN
  ...
  VALUES ('model:Procurement', '0.38.0', 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77', 'action:act_1e35db0451b1461e941af6283d86dca2', v_principal_id, v_idempotency_key, v_request_hash, v_correlation_id, v_causation_id)
  ...
  IF v_receipt_id IS NULL THEN
    ...
    IF v_receipt_source_hash IS DISTINCT FROM 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77' OR v_receipt_request_hash IS DISTINCT FROM v_request_hash THEN
...
;
  ...
  v_response := jsonb_build_object('id', v_result."id", 'createdAt', v_result."created_at", 'requester', v_result."requester_id", 'amount', jsonb_build_object('currency', 'USD', 'amount', (v_result."amount"::numeric(20, 2))::text), 'status', v_result."status", 'approvedBy', v_result."approved_by_id", 'approvedByRoles', v_result."approved_by_roles", 'approvalObserved', v_result."approval_observed");
  INSERT INTO "model_procurement_internal"."action_audit" ("action_id", "database_principal", "principal_id", "target_id", "identity_issuer", "identity_subject", "model_id", "model_version", "source_hash", "authorization_rule_id", "decision_outcome", "policy_id", "authority_id", "decision_evidence", "correlation_id", "causation_id", "command_receipt_id")
  VALUES ('action:act_ed2374e822704c51a2925338253d05d2', session_user, v_principal_id, v_result."id", v_identity_issuer, v_identity_subject, 'model:Procurement', '0.38.0', 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77', 'authorize:action:act_ed2374e822704c51a2925338253d05d2', 'executed', v_authority_policy_id, v_authority_id, pg_catalog.jsonb_build_object('version', 2, 'outcome', 'executed', 'model', pg_catalog.jsonb_build_object('id', 'model:Procurement', 'version', '0.38.0', 'sourceHash', 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77'), 'actionId', 'action:act_ed2374e822704c51a2925338253d05d2', 'command', pg_catalog.jsonb_build_object('correlationId', v_correlation_id, 'causationId', v_causation_id, 'receiptId', v_receipt_id), 'authorization', pg_catalog.jsonb_build_object('ruleId', 'authorize:action:act_ed2374e822704c51a2925338253d05d2', 'outcome', 'passed', 'policyId', v_authority_policy_id, 'authorityId', v_authority_id), 'requirements', pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object('ruleId', 'require:action:act_ed2374e822704c51a2925338253d05d2.is_draft', 'outcome', 'passed', 'policyIds', pg_catalog.jsonb_build_array()))), v_correlation_id, v_causation_id, v_receipt_id)
  RETURNING "id" INTO v_action_audit_id;

  INSERT INTO "model_procurement_internal"."event_outbox" ("model_id", "model_version", "source_hash", "event_id", "event_name", "payload_entity_id", "action_id", "principal_id", "target_id", "payload", "correlation_id", "causation_id", "action_audit_id", "command_receipt_id", "ordinal", "publication_max_attempts", "publication_recovery_mode")
  VALUES ('model:Procurement', '0.38.0', 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77', 'event:evt_20d694c9a0a274dc79c6168e47d25968', 'RequestSubmitted', 'entity:ent_9bc680209327484c8e98f5f740bcc702', 'action:act_ed2374e822704c51a2925338253d05d2', v_principal_id, v_result."id", v_response, v_correlation_id, v_causation_id, v_action_audit_id, v_receipt_id, 0, 5, 'manual');
...
BEGIN
  ...
  IF p_envelope->>'eventId' IS DISTINCT FROM 'event:evt_30d694c9a0a274dc79c6168e47d25968'
     ...
     OR v_source_model_version IS DISTINCT FROM '0.38.0'
     OR v_source_hash IS DISTINCT FROM 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77'
  ...
  VALUES ('model:Procurement', '0.38.0', 'sha256:a4a1c2c6f15a13da6049c57ef6d27ae86de48de2282303fe845c3d5b54e64a77', 'event:evt_50d694c9a0a274dc79c6168e47d25968', 'ApprovalObserved', 'entity:ent_9bc680209327484c8e98f5f740bcc702', 'consumer:con_10d694c9a0a274dc79c6168e47d25968', v_result."id", v_response, v_correlation_id, v_source_event_id::text, v_consumer_audit_id, 0, 5, 'manual');
