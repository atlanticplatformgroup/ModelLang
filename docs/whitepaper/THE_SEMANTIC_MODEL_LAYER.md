...
| General resource templates/subscriptions, full decision traces, delegated capabilities, complete task closure, and agent planning | Proposed; not implemented |
...
|---|---|---|
| Compiler release | 0.42.0 | Toolchain and generated-artifact release |
...
| Example source models | Procurement 0.42.0; Reservations 0.42.0 | Domain-model evolution versions, independent of compiler release |
...
| Agent tool catalog / subject capability view / resource envelope / MCP adapter / task packet | v4 / v1 / v1 / v2 / v1 | Static tools, authenticated exact-input action filtering, query-backed current state, direct MCP transport, and explicitly partial bounded task context; not SML-Agent conformance |
| Target capability profile and extension ledger | v6 / v1 | Deterministic native-support assessment, agent resource, MCP adapter, and task-packet capabilities, plus explicit typed external implementation gaps; none grants authority or generates extension behavior |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.42.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.42 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 300 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
- Target capability profile v6 covers only the canonical PostgreSQL/HTTP/UI/static-agent-catalog/subject-view/resource/MCP/task-packet generator, and the extension ledger does not verify, invoke, deploy, or attest host implementations.

The engineering semantic manifest is not an SML-Agent implementation. It is unfiltered, static, and non-executable. The separate 0.42 public capability manifest v10 is filtered and backed by authenticated side-effect-free applicability, but it covers only declared actions, static reliability/event effects, and safe action-rule IDs; extensions remain absent. Agent tool catalog v4 adds an authenticated exact-input subject overlay, query-backed resource envelope v1, and bounded task-packet binding. MCP adapter v2 directly exposes the tools and packet assembler through stateless Streamable HTTP with exact schemas, per-request audience-bound authentication, and runtime enforcement. Successful query calls carry the resource envelope as distinct embedded content; action calls do not. Packet v1 composes selected static action contracts, evaluated applicability, and caller-selected resources, but deliberately declares partial closure, independent reads, omitted input and identity, zero reusable lifetime, and no authority. Resources preserve declared query disclosure and private read evidence while exposing neither authenticated identity nor evidence. Their freshness is point-in-time with zero reusable lifetime and no-store transport; it is not a commit timestamp, frozen snapshot, or authority grant. Private execution evidence, read-evidence hashes, command receipts, queued event payloads, lease tokens, publication attempts/errors/dispositions/recovery state, consumer inboxes, failure/recovery/observation/acknowledgement/claim state, operator identities, reasons, claimants, outcomes, fingerprints, and results are not published as traces. ModelLang does not yet provide resource templates or subscriptions, prompts, MCP Tasks, public full decision traces, delegated capabilities, complete task goals or observation-relevance proof, full state-write/external-effect/reversibility/recovery closure, nonzero freshness lifetimes, general recovery workflows, cross-context translations, extension-backed tools, or adversarial agent tests. No SML-Federation capabilities are implemented.
