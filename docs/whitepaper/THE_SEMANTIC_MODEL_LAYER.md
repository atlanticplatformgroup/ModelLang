...
|---|---|---|
| Compiler release | 0.32.0 | Toolchain and generated-artifact release |
| Canonical IR | IR21 | Typed backend boundary with acyclic to-one projection traversal and opt-in bound cursor pagination; IR9 through IR21 remain evolution baselines |
| Example source models | Procurement 0.32.0; Reservations 0.32.0 | Domain-model evolution versions, independent of compiler release |
| Operation manifest | v7 | Static transport-neutral public operation, array/page result, and transitive reachable-projection contract |
| UI manifest | v7 | Static framework-neutral presentation, cursor continuation, and nested projection-result contract |
| Enforcement decision plan | v2 | Internal policy- and expression-bearing plan shared by applicability and execution |
| Public capability manifest | v6 | Filtered, expression-free action-applicability, reliability, and event-effect contract that grants no authority |
| Engineering semantic manifest | v13 | Full static semantic closure with nested projection dependencies, cursor-page contracts, and transitive query read/disclosure sets plus event, policy, reliability, and consumer semantics for trusted engineering consumers |
...
| Semantic diff | v14 | Stable-ID pagination-, traversal-, projection-output-, policy-, reliability-, event-, consumer-, failure-, and recovery-aware change analysis that names separate guarded migration authorities |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.32.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.32 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 200 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
