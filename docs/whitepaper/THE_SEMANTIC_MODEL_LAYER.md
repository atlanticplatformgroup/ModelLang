...
| General resource templates/subscriptions, full decision traces, delegated capabilities, and agent planning | Proposed; not implemented |
...
|---|---|---|
| Compiler release | 0.41.0 | Toolchain and generated-artifact release |
...
| Example source models | Procurement 0.41.0; Reservations 0.41.0 | Domain-model evolution versions, independent of compiler release |
...
| Agent tool catalog / subject capability view / resource envelope / MCP adapter | v3 / v1 / v1 / v1 | Static tools, authenticated exact-input action filtering, query-backed current state with conservative freshness, and direct MCP transport; not SML-Agent conformance |
| Target capability profile and extension ledger | v5 / v1 | Deterministic native-support assessment, agent resource and MCP adapter capabilities, and explicit typed external implementation gaps; neither grants authority nor generates extension behavior |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.41.
...
ModelLang 0.41 implements a deliberately narrower precursor to full task closure. The existing closed query system still owns projection disclosure, row policy, authored filters and sorting, pagination, bounds, and optional private transactional evidence. Agent catalog v3 maps each query to an authenticated current-state resource route that returns only the validated query result inside resource envelope v1. MCP adapter v1 exposes every catalog action and query as a stateless Streamable HTTP tool with exact catalog schemas and per-request audience-bound authentication; successful queries additionally carry the same envelope as a distinct embedded resource. The envelope records a transport retrieval time, prohibits storage, declares zero reusable lifetime, and requires revalidation before reuse; it is not a database commit timestamp, frozen snapshot, as-of read, or authority token. Subject action views remain exact-input applicability overlays, and action execution still reloads, locks, and re-enforces current identity, policy, requirements, workflow, invariants, and concurrency controls. Existing reliable commands, transactional events and consumers, bounded failure disposition, isolated recovery, observation, acknowledgement, claiming, and private evidence remain unchanged. This is direct MCP transport for bounded tools and query-backed current state—not a task packet, public trace, delegated capability, general resource template system, or SML-Agent implementation.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.41 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 300 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
- Target capability profile v5 covers only the canonical PostgreSQL/HTTP/UI/static-agent-catalog/subject-view/resource/MCP generator, and the extension ledger does not verify, invoke, deploy, or attest host implementations.

The engineering semantic manifest is not an SML-Agent implementation. It is unfiltered, static, and non-executable. The separate 0.41 public capability manifest v10 is filtered and backed by authenticated side-effect-free applicability, but it covers only declared actions, static reliability/event effects, and safe action-rule IDs; extensions remain absent. Agent tool catalog v3 adds an authenticated exact-input subject overlay for action applicability and query-backed resource envelope v1 for current state. MCP adapter v1 directly exposes those tools through stateless Streamable HTTP with exact catalog schemas, per-request audience-bound authentication, and runtime enforcement. Successful query calls carry the resource envelope as distinct embedded content; action calls do not. Resources preserve declared query disclosure and private read evidence while exposing neither authenticated identity nor evidence. Their freshness is point-in-time with zero reusable lifetime and no-store transport; it is not a commit timestamp, frozen snapshot, or authority grant. Private execution evidence, read-evidence hashes, command receipts, queued event payloads, lease tokens, publication attempts/errors/dispositions/recovery state, consumer inboxes, failure/recovery/observation/acknowledgement/claim state, operator identities, reasons, claimants, outcomes, fingerprints, and results are not published as traces. ModelLang does not yet provide resource templates or subscriptions, prompts, tasks, public full decision traces, delegated capabilities, nonzero freshness lifetimes, general recovery workflows, cross-context translations, agent task packets, extension-backed tools, or adversarial agent tests. No SML-Federation capabilities are implemented.
