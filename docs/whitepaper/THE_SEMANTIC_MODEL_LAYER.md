...
|---|---|---|
| Compiler release | 0.30.0 | Toolchain and generated-artifact release |
| Canonical IR | IR19 | Typed backend boundary with named read projections; IR9 through IR18 remain evolution baselines |
| Example source models | Procurement 0.30.0; Reservations 0.30.0 | Domain-model evolution versions, independent of compiler release |
| Operation manifest | v5 | Static transport-neutral public operation, reachable projection, reliability, and event-effect contract |
| UI manifest | v5 | Static framework-neutral presentation and projection-result contract |
| Enforcement decision plan | v2 | Internal policy- and expression-bearing plan shared by applicability and execution |
| Public capability manifest | v4 | Filtered, expression-free action-applicability, reliability, and event-effect contract that grants no authority |
| Engineering semantic manifest | v11 | Full static semantic closure with projections and distinct query read/disclosure sets plus event, policy, reliability, and consumer semantics for trusted engineering consumers |
...
| Semantic diff | v12 | Stable-ID projection-output-, policy-, reliability-, event-, consumer-, failure-, and recovery-aware change analysis that names separate guarded migration authorities |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.30.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.30 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 200 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
| Stable IDs, typed IR, named closed read projections, reliable commands, typed transactional events and consumers, durable bounded publication/consumer failure disposition, isolated audited publication/consumer recovery, observation, terminal-cycle acknowledgement, and first-writer self-claiming, reusable policies, exact decision evidence, source-linked enforcement, filtered applicability, semantic manifests, semantic diffs, reviewed evolution plans, provenance, and one-way generation are technically implementable | Implemented in the ModelLang 0.30 reference compiler and exercised by its conformance suite | An engineering feasibility claim, not a productivity claim |
...
- Projection traceability covers IR, operation/UI/semantic manifests, OpenAPI, HTTP validation, TypeScript, PostgreSQL enforcement, Mermaid, and evolution analysis, but aliases, traversal, collections, computed fields, and conditional disclosure are not implemented.
