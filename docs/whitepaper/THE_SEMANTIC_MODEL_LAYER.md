...
**Status:** Repository working draft 0.6<br>
**Date:** 5 August 2026
...
|---|---|---|
| Compiler release | 0.44.0 | Toolchain and generated-artifact release |
...
| Example source models | Procurement 0.44.0; Reservations 0.44.0 | Domain-model evolution versions, independent of compiler release |
...
| Agent tool catalog / subject capability view / resource envelope / MCP adapter / task packet / delegated capability / public decision trace | v6 / v1 / v1 / v4 / v1 / v1 / v1 | Static tools, authenticated exact-input action filtering, query-backed current state, direct MCP transport, partial bounded task context, tightly attenuated host-backed action delegation, and bounded current applicability explanation; not SML-Agent conformance |
| Target capability profile and extension ledger | v8 / v1 | Deterministic native-support assessment including delegated-capability and public-trace adapters plus explicit host credential authority, with typed external implementation gaps; discovery grants no authority and no extension behavior is generated |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.44.
...
Typed external-extension declarations now make the remaining handwritten boundary explicit: stable contract, effects, ownership, implementation location, reliability, authorization context, tests, rationale, and promotion criterion are retained in IR and an engineering ledger. A target capability assessment marks these declarations as external implementation gaps while reporting supported core semantics as native. Neither artifact creates a callable, verifies a host implementation, or grants authority.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.44 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 300 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
