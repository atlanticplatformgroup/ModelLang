...
| Static agent tool catalog with closed schemas, exact HTTP bindings, safe applicability metadata, and runtime-enforcement flags | Implemented as a non-authoritative MCP-adaptable contract |
| Authenticated subject-specific action capability views | Implemented as an exact-input applicability overlay; no resource state, authority grant, or MCP server |
...
|---|---|---|
| Compiler release | 0.39.0 | Toolchain and generated-artifact release |
...
| Example source models | Procurement 0.39.0; Reservations 0.39.0 | Domain-model evolution versions, independent of compiler release |
...
| Agent tool catalog / subject capability view | v2 / v1 | Static expression-free action/query schemas plus authenticated exact-input action filtering; MCP-adaptable but not direct MCP or SML-Agent conformance |
| Target capability profile and extension ledger | v3 / v1 | Deterministic native-support assessment, static-catalog and subject-view capabilities, and explicit typed external implementation gaps; neither grants authority nor generates extension behavior |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.39.
...
ModelLang 0.39 implements a deliberately narrower precursor to this closure. Stable named read projections make query disclosure explicit, `redactable` members and query-local Boolean rules add static fail-closed value disclosure while preserving required JSON keys, acyclic to-one dependencies add compile-time-bounded nested shapes with exact null behavior, optional query inputs require explicit authored null-aware predicates, closed authored sort profiles compile to static ordering branches, and opt-in fixed-limit keyset cursors bind continuation to model/query identity, source hash, disclosure revision, selected profile, principal, and filters without granting authority; hidden predicate, disclosure, and ordering fields remain enforcement inputs, and shape reuse grants no authority. A filtered public capability manifest names action inputs, static reliability requirements, declared action-emitted event IDs, and safe stable rule IDs without publishing compiler expressions, policy identities, command/event instances, consumers, inboxes, failure/recovery state, or current state. Static agent catalog v2 projects declared actions and queries into stable identities, standalone closed JSON schemas, exact authenticated HTTP routes, fixed applicability metadata, reliability/event declarations, and bounded query metadata. Its authenticated subject view evaluates up to 32 distinct exact action candidates for one bearer context through the same generated applicability boundary, returning only stable action outcomes and opaque revisions without echoing identity, input, or resource state; it grants no authority, promises no atomic snapshot or freshness lifetime, and leaves queries static-catalog-only. New action execution reloads, locks, and re-evaluates the plan, then privately records exact stable policy authority with action audit; explicitly reliable actions complete a principal-scoped receipt, and declared events append their typed post-effect payloads plus copied static publication policy to a private outbox in the same transaction. Lease-bound publication failures can reach a broker-neutral private terminal disposition without network publication, routing, or inferred crash failures. Opted-in terminal outbox instances can be restored to normal dispatcher claim eligibility by a separate operational role with current-cycle reset, monotonic total failures, generation, and exact private audit; that recovery does not claim, publish, reconstruct, or move a broker message. Separately, stable typed consumers validate exact source contracts and use a private transactional inbox to serialize duplicate delivery and replay one committed local result. A consumer may append local downstream events atomically with that result; correlation is inherited, causation identifies the consumed source event, duplicate replay emits nothing, and compile-time cycle rejection keeps the local event graph acyclic. Optional bounded consumer failure policy adds private durable attempt accounting and a broker-neutral private terminal disposition without controlling the broker. Opt-in manual recovery lets an isolated operational role reopen one consumer terminal identity with exact private audit, but invokes no handler and performs no broker operation. A third operational role may traverse a bounded, minimally projected, privately audited view of current terminal publication and consumer failures; observation grants neither recovery nor dispatch authority and is not exposed to public or agent consumers. A fourth role may append one immutable acknowledgement for the current terminal recovery generation under the same failure-state lock as recovery; it changes no failure, delivery, broker, or domain state, and observation exposes only a current-generation Boolean. A fifth isolated role may become the immutable first claimant for one current terminal generation under that same lock; later attempts disclose only that a claim exists, and observation adds only a second Boolean. This remains an application-facing preflight contract plus bounded subject-specific action discovery and internal audit, retry, delivery, publication, consumption, chain, failure, recovery, observation, acknowledgement, and claim evidence—not a current-state agent resource, task packet, public trace, delegated capability, or SML-Agent implementation.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.39 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 300 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
- Target capability profile v3 covers only the canonical PostgreSQL/HTTP/UI/static-agent-catalog/subject-view generator, and the extension ledger does not verify, invoke, deploy, or attest host implementations.

The engineering semantic manifest is not an SML-Agent implementation. It is unfiltered, static, and non-executable. The separate 0.39 public capability manifest v10 is filtered and backed by authenticated side-effect-free applicability, but it covers only declared actions, static reliability/event effects, and safe action-rule IDs; extensions remain absent. Agent tool catalog v2 adds static action/query identities, closed schemas, HTTP bindings, filtered metadata, and an authenticated exact-input subject overlay for action applicability. The overlay contains no resource payload, identity, policy expression, atomic snapshot, or authority grant; query tools remain static-only, and no direct MCP protocol is implemented. Private execution evidence, read-evidence hashes, command receipts, queued event payloads, lease tokens, publication attempts/errors/dispositions/recovery state, consumer inboxes, failure/recovery/observation/acknowledgement/claim state, operator identities, reasons, claimants, outcomes, fingerprints, results, and evidence are not published as traces. ModelLang does not yet provide general authorization-filtered resource views, public full decision traces, delegated capabilities, freshness lifetimes, general recovery workflows, cross-context translations, agent task packets, or adversarial agent tests. No SML-Federation capabilities are implemented.
