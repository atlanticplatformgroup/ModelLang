...
|---|---|---|
| Compiler release | 0.34.0 | Toolchain and generated-artifact release |
| Canonical IR | IR23 | Typed backend boundary with closed authored sort profiles, optional authored query filters, acyclic to-one projection traversal, and opt-in bound cursor pagination; IR9 through IR23 remain evolution baselines |
| Example source models | Procurement 0.34.0; Reservations 0.34.0 | Domain-model evolution versions, independent of compiler release |
| Operation manifest | v9 | Static transport-neutral public operation, optional query-input, closed sort-profile, array/page result, and transitive reachable-projection contract |
| UI manifest | v9 | Static framework-neutral presentation, optional filter, closed sort choices, cursor continuation, and nested projection-result contract |
| Enforcement decision plan | v2 | Internal policy- and expression-bearing plan shared by applicability and execution |
| Public capability manifest | v8 | Filtered, expression-free action-applicability, reliability, and event-effect contract that grants no authority |
| Engineering semantic manifest | v15 | Full static semantic closure with optional query inputs, authored sort profiles, nested projection dependencies, cursor-page contracts, and transitive query read/disclosure sets plus event, policy, reliability, and consumer semantics for trusted engineering consumers |
...
| Semantic diff | v16 | Stable-ID query-input-, sort-profile-, pagination-, traversal-, projection-output-, policy-, reliability-, event-, consumer-, failure-, and recovery-aware change analysis that names separate guarded migration authorities |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.34.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.34 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 200 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
| Stable IDs, typed IR, named closed read projections with bounded to-one traversal, optional filters, authored sort profiles, reliable commands, typed transactional events and consumers, durable bounded publication/consumer failure disposition, isolated audited publication/consumer recovery, observation, terminal-cycle acknowledgement, and first-writer self-claiming, reusable policies, exact decision evidence, source-linked enforcement, filtered applicability, semantic manifests, semantic diffs, reviewed evolution plans, provenance, and one-way generation are technically implementable | Implemented in the ModelLang 0.34 reference compiler and exercised by its conformance suite | An engineering feasibility claim, not a productivity claim |
...
- Projection traceability covers IR, operation/UI/semantic manifests, OpenAPI, HTTP validation, TypeScript, PostgreSQL enforcement, Mermaid, and evolution analysis, but aliases, collections, reverse traversal, computed fields, and conditional disclosure are not implemented.
