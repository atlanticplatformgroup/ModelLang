...
|---|---|---|
| Compiler release | 0.35.0 | Toolchain and generated-artifact release |
| Canonical IR | IR24 | Typed backend boundary with conditional field disclosure, closed authored sort profiles, optional authored query filters, acyclic to-one projection traversal, and opt-in bound cursor pagination; IR9 through IR24 remain evolution baselines |
| Example source models | Procurement 0.35.0; Reservations 0.35.0 | Domain-model evolution versions, independent of compiler release |
| Operation manifest | v10 | Static transport-neutral public operation, null-redaction, optional query-input, closed sort-profile, array/page result, and transitive reachable-projection contract |
| UI manifest | v10 | Static framework-neutral presentation, conditional-disclosure metadata, optional filters, closed sort choices, cursor continuation, and nested projection-result contract |
| Enforcement decision plan | v2 | Internal policy- and expression-bearing plan shared by applicability and execution |
| Public capability manifest | v9 | Filtered, expression-free action-applicability, reliability, and event-effect contract that grants no authority |
| Engineering semantic manifest | v16 | Full static semantic closure with conditional-disclosure rules, optional query inputs, authored sort profiles, nested projection dependencies, cursor-page contracts, and transitive query read/disclosure sets plus event, policy, reliability, and consumer semantics for trusted engineering consumers |
...
| Semantic diff | v17 | Stable-ID field-disclosure-, query-input-, sort-profile-, pagination-, traversal-, projection-output-, policy-, reliability-, event-, consumer-, failure-, and recovery-aware change analysis that names separate guarded migration authorities |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.35.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.35 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, and more than 300 automated conformance tests. This establishes engineering feasibility for the implemented subset; it does not establish that the architecture improves software delivery.
...
| Stable IDs, typed IR, named closed read projections with conditional field disclosure and bounded to-one traversal, optional filters, authored sort profiles, reliable commands, typed transactional events and consumers, durable bounded publication/consumer failure disposition, isolated audited publication/consumer recovery, observation, terminal-cycle acknowledgement, and first-writer self-claiming, reusable policies, exact decision evidence, source-linked enforcement, filtered applicability, semantic manifests, semantic diffs, reviewed evolution plans, provenance, and one-way generation are technically implementable | Implemented in the ModelLang 0.35 reference compiler and exercised by its conformance suite | An engineering feasibility claim, not a productivity claim |
