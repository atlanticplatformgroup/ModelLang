...
**Status:** Repository working draft 0.9<br>
**Date:** 6 August 2026
...
| Stable named read projections with closed output objects and hidden predicate fields | Implemented for direct non-collection source fields across PostgreSQL, OpenAPI, HTTP validation, TypeScript, UI, semantic closure, and evolution analysis |
...
| Ordered atomic multi-entity command effects with affected-record evidence | Implemented for PostgreSQL-local update/create sequences; all guards, effects, evidence, receipts, and emitted events share one transaction |
...
| Separately authorized terminal-cycle acknowledgement | Implemented as one immutable acknowledgement per current recovery generation, serialized with recovery and reflected only as a Boolean in private observation; failure, delivery, broker, and domain state remain unchanged |
| Separately authorized terminal-cycle self-claim | Implemented as one immutable first-writer claimant per current recovery generation, serialized with recovery and reflected only as a Boolean in private observation; assignment transfer and workflow ownership remain absent |
...
| Static agent tool catalog with closed schemas, exact HTTP bindings, safe applicability metadata, and runtime-enforcement flags | Implemented as a non-authoritative catalog and direct MCP tool source |
| Authenticated subject-specific action capability views | Implemented as an exact-input applicability overlay with no authority grant |
...
| Extension-backed agent tool adapters | Implemented as stable exact-schema HTTP/MCP bindings that require exact-revision host registration and per-call host authorization; external behavior, effects, evidence, tests, deployment, and verification remain host-owned, and every implementation gap remains open |
| Adversarial agent assurance and SML-Agent assessment | Implemented as a deterministic hostile-client suite, conservative ten-criterion partial assessment, four-condition scenario/replay format, and non-empirical deterministic scorer; no complete-conformance, agent-competence, test-execution, or benchmark-result claim |
| General resource templates/subscriptions, historical or full decision traces, transferable or chained delegation, complete task closure, and agent planning | Proposed; not implemented |
...
|---|---|---|
| Compiler release | 0.50.0 | Atomic multi-entity effects and canonical IR2 |
| npm distribution | `modellang@0.50.0` | Apache-2.0 public-preview package with `modelc`, compiled compiler modules, schemas, and release documentation |
...
| Public capability manifest | v10 | Filtered, expression-free action-applicability, reliability, and event-effect contract that grants no authority |
| Engineering semantic manifest | v19 | Full static semantic closure including typed external-extension contracts and the prior read, event, policy, reliability, and consumer semantics for trusted engineering consumers |
...
| Semantic diff | v20 | Stable-ID ordered-effect-, extension-, read-evidence-, disclosure-, query-, policy-, reliability-, event-, consumer-, failure-, and recovery-aware change analysis that names separate guarded migration authorities |
| Agent tool catalog / subject capability view / resource envelope / MCP adapter / task packet / delegated capability / public decision trace / extension tool result | v7 / v1 / v1 / v6 / v1 / v1 / v1 / v1 | Static core and extension tools, authenticated exact-input action filtering, query-backed current state, revision-bound static MCP discovery caching, partial bounded task context, tightly attenuated host-backed action delegation, bounded current applicability explanation, and host-asserted extension results; not SML-Agent conformance |
| Agent Plugins package format | 1.0.0 Working Draft | Optional deployment wrapper with portable identity and one remote MCP connection; not a ModelLang language, IR, authority, or wire-protocol version |
| SML-Agent assessment / agent evaluation suite / replay | v1 / v1 / v1 | Conservative engineering assessment plus deterministic scenario and observation formats; synthetic fixtures and scorer reports make no empirical claim |
| Target capability profile and extension ledger | v9 / v1 | Deterministic native adapter-support assessment including extension-tool adapters, with typed external implementation gaps; discovery grants no authority and no extension behavior is generated or verified |
...
This paper proposes the **semantic model layer**: a versioned, typed, executable representation of a bounded application domain that defines concepts, stable identity, relationships, valid states, permitted transitions, policies, queries, events, effects, and presentation intent. The layer is architectural rather than necessarily a runtime service. A complete implementation could translate it into database schemas and migrations, backend handlers, API contracts, frontend metadata, policy checks, agent tools, tests, and documentation. The status table above identifies the smaller subset implemented by ModelLang 0.50.
...
ModelLang 0.50 retains the explicitly partial task-packet precursor to full task closure. An authenticated caller selects exact declared action candidates and bounded declared query observations. Assembly reuses current action applicability and query execution: the query system still owns projection disclosure, row policy, authored filters and sorting, pagination, bounds, and optional private transactional evidence, while action applicability retains identity, policy, requirement, workflow, and concurrency-revision evaluation. Task packet v1 publishes selected static action schemas, safe failure/reliability/event/workflow metadata, current applicability decisions, and unchanged resource envelope v1 observations under opaque bindings. It omits the supplied action and observation values and authenticated identity, never executes an action, declares observations as independent rather than atomic, grants no authority, has zero reusable lifetime, and names its missing declaration-identity closure, goal, relevance, complete state-write/external-effect, reversibility, and recovery closure. Agent catalog v7 binds the HTTP route; MCP adapter v6 exposes the same exact assembler as a read-only tool and embedded packet resource without advertising MCP Tasks. Later action execution still reloads, locks, and re-enforces every current rule.

Delegated capability v1 is separate from task context and discovery. An authenticated grantor may issue one opaque credential for one currently applicable exact action/input hash, one named authenticated delegate, one audience, one required concurrency revision, one use, and at most one hour. The delegate must present ordinary audience-bound authentication plus the separate credential. Generated HTTP and MCP adapters validate the closed attenuation contract and reject transfer to reads, resources, applicability, packets, traces, issuance, or revocation. The host owns credential entropy and storage, grantor/delegate binding, revocation, and atomic consume-and-execute; the resulting call re-enters the stored grantor's normal action executor so current policy, preconditions, revision, workflow, locks, invariants, evidence, and output validation remain authoritative. This is bounded delegated action authority—not a bearer replacement, role assignment, delegation chain, complete task closure, general MCP resource system, or SML-Agent implementation.

Public decision trace v1 is also separate from authority and private evidence. For one exact authenticated action candidate, it reuses the current applicability evaluator and publishes the existing safe decision plus ordered categorical outcomes for authorization, requirements, and optional revision matching. It omits the supplied input, all evaluated state values, authenticated identity, expressions, policy and authority identities, SQL, private `decision_evidence`, and execution results. It observes no action execution and writes no action-audit row. HTTP and MCP responses declare applicability-only incomplete closure, `authority: "none"`, point-in-time transport time, zero reusable lifetime, revalidation before reuse, and no-store handling. This is a bounded current explanation—not a history, durable proof, public copy of successful-execution evidence, full decision trace, or authority grant.

Opted-in transactional read evidence additionally binds successful committed reads to authenticated direct or gateway identity, a deterministic query revision, canonical request SHA-256, and exact response SHA-256 without retaining inputs, cursors, or returned payloads. The record shares the caller transaction, so rollback removes it; this is a committed-read audit boundary rather than an autonomous, signed, or externally attested trace.

Typed external-extension declarations make the remaining handwritten boundary explicit: stable contract, effects, ownership, implementation location, reliability, authorization context, tests, rationale, and promotion criterion are retained in IR and a private engineering ledger. ModelLang 0.50 retains a separate public tool binding with exact schemas and an opaque semantic contract revision. Generated HTTP and MCP adapters require the authenticated host to affirm that exact revision, authorize the validated input, and supply the implementation. The host owns downstream identity, effects, retries, evidence, tests, deployment, and monitoring; the compiler verifies none of them. The ledger remains non-executable with zero generated implementations, and the target continues to report every external implementation gap. This creates a callable adapter boundary, not generated behavior, attestation, authority, or task closure.

ModelLang 0.50 also retains a conservative SML-Agent assessment rather than promoting these partial contracts into a conformance claim. The assessment maps the ten Appendix B.2 requirements to four supported and six partial criteria for the current reference models, cites exact contract/runtime evidence, and names missing authorization-filtered discovery, missing-fact planning, complete effects/reversibility/recovery closure, full decision traces, host extension evidence, and executed-test attestation. It always declares partial status, no authority, no agent-competence claim, and no included live-model evidence. A deterministic hostile-client suite exercises identity, staleness, metadata, delegation, disclosure, request isolation, and operation-kind confusion over unit and live PostgreSQL/MCP boundaries.

MCP adapter v6 separates static discovery caching from semantic freshness. A deployment may select a private TTL for `server/discover` and `tools/list`; the compiler supplies a deterministic revision over the exact generated discovery contract and the server returns it as an `ETag`. Zero remains the default. Current-state resources, task packets, applicability traces, actions, delegated invocations, extensions, and errors remain no-store, and every operation independently authenticates and re-enforces current rules. A cached tool description therefore reduces repeated catalog transfer but is neither evidence that business state is unchanged nor proof that the caller may use the tool.

ModelLang 0.50 can wrap a deployed generated application in the Agent Plugins 1.0.0 Working Draft package format. A deployment option supplies the public MCP URL; the compiler emits a separate package root with portable identity and one Streamable HTTP connection while keeping the existing ModelLang adapter manifest intact. The endpoint and package name stay outside source and IR, and generated configuration contains no credentials or authentication headers. This improves cross-client installation only. Plugin discovery, installation, or connection is not an authority grant, so client-managed authentication and the same per-request ModelLang enforcement remain mandatory.

The 0.50 compiler itself is also packaged as the Apache-2.0 public-preview npm artifact `modellang`, exposing the `modelc` executable on Node.js 20 or newer. Package conformance is tested on the actual tarball: repository-only files are excluded, the artifact is installed into an empty consumer, canonical IR schema lookup resolves inside the installed package, and the installed CLI checks and builds a model including its optional Agent Plugin wrapper. CI covers the supported Node range and a live PostgreSQL service; tag-driven automation repeats those gates before npm publication. Package installation and supply-chain provenance improve distribution assurance but remain unrelated to application identity or authority.

The repository further provides machine-readable versions of the four evaluation conditions and initial tasks proposed in Section 9.6 and Appendix C.7, a provider-neutral driver interface, replay records, and deterministic scoring. The committed replay is synthetic and exists only to verify the scorer; every report is labeled non-empirical. These mechanisms make a later controlled experiment reproducible, but they are not themselves evidence that semantic context improves agent planning.
...
This document is a research proposal and architecture design. It does not present pilot results, production telemetry, controlled experiments, or longitudinal adoption data. ModelLang 0.50 provides two executable reference applications, deterministic generated golden artifacts, live PostgreSQL integration coverage, a hostile-client assurance suite, and more than 300 automated conformance tests. Its agent-evaluation replay is synthetic and non-empirical. This establishes engineering feasibility for the implemented subset and scorer; it does not establish that the architecture improves software delivery or agent performance.
...
| Stable IDs, typed IR, declared external-extension obligations, canonical target capability assessment, named closed read projections with conditional field disclosure, transactional read evidence, and bounded to-one traversal, optional filters, authored sort profiles, reliable commands, typed transactional events and consumers, durable bounded publication/consumer failure disposition, isolated audited publication/consumer recovery, observation, terminal-cycle acknowledgement, and first-writer self-claiming, reusable policies, exact decision evidence, source-linked enforcement, filtered applicability, authenticated subject action views, query-backed current-state agent resources, stateless MCP adapter integration with revision-bound static discovery caching, optional portable Agent Plugin packaging, clean-installable public compiler distribution, explicitly partial bounded task packets, tightly bounded host-backed delegated action capabilities, bounded public applicability traces, exact-revision host extension-tool adapters, conservative agent assessment, deterministic hostile-client assurance, semantic manifests, semantic diffs, reviewed evolution plans, provenance, and one-way generation are technically implementable | Implemented in the ModelLang 0.50 reference compiler and exercised by its conformance suite | An engineering feasibility claim, not a productivity, agent-quality, compatible-client, npm-publication, or host-implementation-verification claim |
...
## A.5 Status in the 0.29 reference implementation
...
| Current terminal failures could not be discovered without private-table access | Fixed in 0.27 with a separately authorized bounded observer, minimal keyset projection, and exact private inspection audit |
| Operators could not durably mark one terminal recovery generation as seen | Fixed in 0.28 with separately authorized immutable acknowledgement serialized with recovery; private observation exposes only the current-generation Boolean |
| Operators could not privately establish one first claimant for a terminal generation | Fixed in 0.29 with separately authorized immutable self-claiming serialized with recovery; private observation exposes only the current-generation Boolean |

ModelLang 0.29 retains the reviewed evolution artifact, filtered public capability manifest, runtime applicability decisions, reliable commands, private transactional execution evidence, audited recovery, bounded lease-bound failure disposition, bounded private observation, and immutable acknowledgement. It adds separately authorized immutable first-writer self-claiming of one current terminal recovery generation without changing acknowledgement, failure, delivery, broker, or domain state. The observer exposes only `acknowledged` and `claimed` for the current generation; reasons, operators, claimants, audit identity, and history remain private. Payloads, principals, correlations, causation, decision evidence, receipts, fingerprints, stored responses, leases, and broker state remain excluded, and observation, acknowledgement, or claiming grants no recovery or dispatch authority. Event manifest v5 exposes only static publication failure/recovery policy. The engineering manifest remains trusted static analysis, the reviewed plan remains migration intent, applicability grants no execution authority, and durable audit, command, event, publication, consumer, failure, recovery, observation, acknowledgement, or claim evidence is not a capability token.
...
## B.5 ModelLang 0.50 conformance declaration

ModelLang 0.50 does not claim complete conformance with SML-Core. It substantially implements model and declaration identity for its current language, typed references and values, valid-state semantics, reusable closed policies, reliable PostgreSQL-local commands, typed post-effect domain events with private at-least-once delivery, bounded failure disposition, isolated audited terminal reopening, separately authorized bounded terminal-failure observation, single-cycle acknowledgement, and first-writer self-claiming, stable typed consumers with duplicate-safe PostgreSQL-local committed handling, acyclic local downstream emission, exact action and consumer evidence, action semantics, explicit workflows, stable named projections with bounded acyclic to-one traversal and conditional field disclosure and transactional read evidence, operation-level and row-level query visibility, optional authored query filters, closed authored sort profiles, fixed-limit cursor continuation, typed external-extension obligations, canonical target capability assessment, typed IR, deterministic diagnostics, PostgreSQL-oriented traceability, semantic change analysis, reviewed evolution intent, authenticated applicability, authenticated subject-specific exact-input action views, query-backed current-state resources with zero-age freshness, stateless direct MCP adapter transport with revision-bound static discovery caching, optional portable Agent Plugin packaging, clean-installable public compiler distribution, explicitly partial bounded task packets, tightly bounded delegated action capabilities, bounded public applicability traces, exact-revision host extension-tool adapters, conservative agent assessment, deterministic hostile-client assurance, and conformance tests.
...
- Policy v1 is deliberately Boolean and closed; external calls from core expressions, structured obligations, and custom event payload mappings are not language features. Extension contracts are declaration-only and cannot be invoked by the core.
- Projection traceability covers IR, operation/UI/semantic manifests, OpenAPI, HTTP validation, TypeScript, PostgreSQL enforcement, Mermaid, and evolution analysis, including static null-redaction rules; aliases, collections, reverse traversal, computed fields, key omission, redaction reasons, and runtime field selection are not implemented.
...
- Target capability profile v9 covers only the canonical PostgreSQL/HTTP/UI/static-agent-catalog/subject-view/resource/MCP/task-packet/delegated-capability/public-applicability-trace/extension-tool adapter generator. Delegated credential storage and atomic consumption are a required host conformance boundary, and neither that host implementation nor extension implementations are generated, deployed, verified, or attested.

The engineering semantic manifest is not an SML-Agent implementation. It is unfiltered, static, and non-executable. The separate 0.50 public capability manifest v10 is filtered and backed by authenticated side-effect-free applicability, but it covers only declared actions, static reliability/event effects, and safe action-rule IDs; extensions remain absent from that capability contract. Agent tool catalog v7 adds an authenticated exact-input subject overlay, query-backed resource envelope v1, bounded task-packet binding, delegated capability v1 metadata, public decision trace v1, and a separate extension-tool collection. MCP adapter v6 directly exposes core tools, resources, packets, public traces, delegated action invocation, and host-backed extension tools through stateless Streamable HTTP with exact schemas, per-request audience-bound authentication, and runtime enforcement; delegated issuance remains HTTP-only. Its private deployment-selected TTL and deterministic revision apply only to static `server/discover` and `tools/list` results. The optional Agent Plugins 1.0.0 package only identifies and locates this deployed endpoint; it contains no credential or authority and does not change MCP behavior. Every tool invocation still authenticates independently, and current-state, applicability, packet, trace, delegation, extension, and action results remain no-store. Each extension call requires exact-revision host registration and host authorization, returns a no-store non-authoritative result envelope, and creates no MCP resource. The compiler does not generate or verify extension behavior, effects, evidence, or tests, and the target implementation gap remains open. Successful query calls carry the resource envelope as distinct embedded content; action and extension calls do not. Packet v1 composes selected static action contracts, evaluated applicability, and caller-selected resources, but deliberately declares partial closure, independent reads, omitted input and identity, zero reusable lifetime, and no authority. Resources preserve declared query disclosure and private read evidence while exposing neither authenticated identity nor evidence. Their freshness is point-in-time with zero reusable lifetime and no-store transport; it is not a commit timestamp, frozen snapshot, or authority grant. Delegated capability v1 grants only one exact revision-bound action/input to one named authenticated delegate, one audience, once, for at most one hour; host-owned storage, revocation, identity binding, and atomic consume-and-execute are mandatory. Public decision trace v1 reports only the current safe applicability decision and ordered categorical authorization, requirement, and revision outcomes; it omits inputs, evaluated values, identity, expressions, policy/authority identities, private evidence, and execution claims, and grants no authority. Assessment v1 records four supported and six partial SML-Agent criteria for the current examples, but it is a non-authoritative, non-attesting engineering artifact. The deterministic hostile-client suite and synthetic scorer fixture establish executable assurance mechanics, not language-model performance or complete profile coverage. Private execution evidence, read-evidence hashes, command receipts, queued event payloads, lease tokens, publication attempts/errors/dispositions/recovery state, consumer inboxes, failure/recovery/observation/acknowledgement/claim state, operator identities, reasons, claimants, outcomes, fingerprints, and results are not published. ModelLang does not yet provide resource templates or subscriptions, prompts, MCP Tasks, historical or full public decision traces, transferable or chained delegation, complete task goals or observation-relevance proof, full state-write/external-effect/reversibility/recovery closure, nonzero current-state freshness lifetimes, general recovery workflows, cross-context translations, generated or verified extension implementations, delegated extension authority, or controlled comparative agent evidence. No complete SML-Agent or SML-Federation conformance is claimed.
