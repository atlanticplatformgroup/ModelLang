...
describe("backends", () => {
  ...
  it("emits engineering-only semantic closure and deterministic artifact provenance", async () => {
    ...
    expect(target.capabilities).toContainEqual({
      id: "agents.publicDecisionTraces",
      required: true,
      support: "native",
  ...
  it("emits a schema-valid MCP adapter with exact catalog schemas and distinct query resources", async () => {
    ...
    expect(manifest.publicDecisionTrace).toMatchObject({
    ...
    const publicTraceSchema = JSON.parse(await readFile("schemas/public-decision-trace.schema.json", "utf8")) as object;
    expect(() => new Ajv2020({
      allErrors: true,
      strict: true,
      formats: { uuid: true, "date-time": true },
    }).compile(publicTraceSchema)).not.toThrow();
    for (const exactSchema of [manifest.publicDecisionTrace.inputSchema, manifest.publicDecisionTrace.outputSchema]) {
      expect(() => new Ajv2020({
        allErrors: true,
        strict: true,
        formats: { uuid: true, "date-time": true },
...
query active(caller actor: User) returns RecordSummary from Record as row {
  ...
  it("generates opaque keyset cursor pages bound to query and filter identity", async () => {
    ...
    expect(sql).toContain("'modelVersion', '0.49.0'");
  ...
  it("generates database workflow backstops, typed metadata, and lifecycle diagrams", async () => {
    ...
    expect(schema).toContain("VALUES ('model:Procurement', '0.49.0'");
