# ModelLang 0.35 reference compiler
...
`capabilities.json` is public capability manifest v9, a filtered projection derived from the operation manifest and decision plan. It exposes action/input and emitted-event IDs, static reliability requirements, fixed applicability outcomes, safe explanation rule IDs, and opaque-revision behavior. It contains no command or event instances, keys, correlations, receipts, expressions, current state, SQL details, or authority grant.

`semantic.json` is engineering semantic manifest v16. It includes every projection and nested dependency, marks redactable members, records query disclosure paths, expressions, dependencies, and policy usage, distinguishes required and optional query inputs, records authored query sort profiles, preserves bounded array and cursor-page contracts, separates transitive query source read sets from disclosed shapes, and retains stable event publication failure and recovery policies plus consumers, accepted source contracts, delivery identity, bounded or unbounded consumer failure policy, recovery eligibility, rules, lock sets, local effects, and ordered downstream event IDs.

`ui.json` is UI manifest v10 derived exclusively from operation manifest v10. Query descriptors mark authored filters as required/non-null or optional/nullable, expose closed sort-profile choices, cursor metadata, and fail-closed disclosure rules where declared, and use `resultProjectionId`; projection fields retain explicit nested-projection IDs and redaction eligibility. Entity descriptors remain available for action results and reference inputs but grant no read capability.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, full-text search, and read-audit policy. ModelLang 0.35 supports explicit optional scalar, enum, money, and entity filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, and opt-in fixed-limit keyset cursors.
...
- UI manifest v10 intentionally omits framework components, layout, localization, entity option queries, authorization visibility/preflight, generic CRUD, prescribed pagination controls, optimistic concurrency, and client-side validation policy. It describes filter nullability, sort choices, cursor continuation, and null-redaction metadata but leaves rendering to consumers. Alternate transports and AI/MCP generation remain deferred consumers of declared operations.
- Engineering semantic manifest v16 is intentionally a trusted static artifact, not an authorization-filtered capability view. Public policy traces, freshness lifetimes, general recovery workflows, external operations, extensions, target capability profiles, and agent/MCP generation remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.35 language is in [spec/0.35/LANGUAGE.md](./spec/0.35/LANGUAGE.md), with its [conditional field-disclosure contract](./spec/0.35/CONDITIONAL_FIELD_DISCLOSURE.md), [conformance requirements](./spec/0.35/CONFORMANCE.md), and [unstable boundaries](./spec/0.35/UNSTABLE.md). Earlier sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.35 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
