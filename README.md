# ModelLang 0.37 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
Each model has a generated subtree: `generated/procurement/` and `generated/reservations/`. Its `model.ir.json` is the only compiler-backend input. ModelLang 0.37 advances to IR26 with typed external-extension obligations and target capability assessment while preserving transactional read evidence and the existing closed read family. IR9 through IR26 remain accepted evolution baselines; historical models gain neither invented extension obligations nor fabricated evidence. Both committed subtrees are golden fixtures and migration baselines.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v1: it marks required core semantics as native and every extension as an external implementation gap; it grants no authority.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.37 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, and opt-in committed private read evidence.
...
The normative 0.37 language is in [spec/0.37/LANGUAGE.md](./spec/0.37/LANGUAGE.md), with its [target-capability and extension-ledger contract](./spec/0.37/TARGET_CAPABILITIES_AND_EXTENSIONS.md), [conformance requirements](./spec/0.37/CONFORMANCE.md), and [unstable boundaries](./spec/0.37/UNSTABLE.md). Earlier read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.37 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
