# ModelLang 0.45 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static agent tool catalog with authenticated subject-specific action views, current-state query resources, bounded task packets, exact single-use delegated action capabilities, bounded public applicability traces, and host-adapter bindings for declared external extensions, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
`mcp.json` is MCP adapter manifest v5. It exposes every catalog action and query as a stable MCP tool with exact schemas, `modellang_task_packet` as a distinct read-only assembler, `modellang_public_decision_trace` as a separate read-only applicability explanation, and declared extensions as separate host-backed tools. The generated server-only `typescript/mcp-server.ts` implements authenticated stateless Streamable HTTP. Each extension invocation requires the request-bound host to affirm its exact contract revision and authorize the validated input; missing registration fails closed. Query, packet, and trace calls return embedded resources, while action and extension calls remain executable tool results. Extension results use no-store metadata but carry no current-state resource freshness claim. Resource templates, subscriptions, prompts, MCP Tasks, generated extension implementations, complete task closure, and full SML-Agent conformance are not claimed.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.45 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v9 reports the static catalog, bounded subject action view, query-backed current-state resources, direct MCP adapter, explicitly partial task packets, bounded delegated-capability adapter, public applicability traces, and extension-tool adapter as native. Host credential authorities and extension implementations remain required, ungenerated, and unattested. The ledger remains non-executable with zero generated implementations, and target gaps remain open. Automatic observation selection, complete effect/recovery closure, MCP resource templates and subscriptions, prompts, MCP Tasks, transferable or chained delegation, historical or complete public traces, delegated extension authority, adversarial agent testing, host implementation verification, alternate target generators, cross-target conformance, nonzero freshness lifetimes, read-evidence observation, and general recovery workflows remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.45 language is in [spec/0.45/LANGUAGE.md](./spec/0.45/LANGUAGE.md), with its [extension-tool adapter contract](./spec/0.45/EXTENSION_TOOLS.md), [conformance requirements](./spec/0.45/CONFORMANCE.md), and [unstable boundaries](./spec/0.45/UNSTABLE.md). Earlier public-trace, delegated-capability, task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.45 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
