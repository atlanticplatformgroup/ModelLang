# ModelLang 0.36 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, private transactional action and read evidence, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
Each model has a generated subtree: `generated/procurement/` and `generated/reservations/`. Its `model.ir.json` is the only compiler-backend input. ModelLang 0.36 advances to IR25 with opt-in transactional read evidence while preserving fail-closed conditional disclosure, closed authored sort profiles, optional query filters, opaque keyset cursors, bounded to-one traversal, and unpaginated array results. IR9 through IR25 remain accepted evolution baselines; historical models gain neither invented read-audit guarantees nor fabricated evidence. Both committed subtrees are golden fixtures and migration baselines.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.36 supports explicit optional scalar, enum, money, and entity filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, and opt-in committed private read evidence.
...
- Engineering semantic manifest v17 is intentionally a trusted static artifact, not an authorization-filtered capability view. Public policy traces, freshness lifetimes, read-evidence observation, general recovery workflows, external operations, extensions, target capability profiles, and agent/MCP generation remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.36 language is in [spec/0.36/LANGUAGE.md](./spec/0.36/LANGUAGE.md), with its [transactional read-evidence contract](./spec/0.36/TRANSACTIONAL_READ_EVIDENCE.md), [conformance requirements](./spec/0.36/CONFORMANCE.md), and [unstable boundaries](./spec/0.36/UNSTABLE.md). Earlier disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.36 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
