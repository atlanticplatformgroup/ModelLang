# ModelLang 0.42 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static agent tool catalog with authenticated subject-specific action views, current-state query resources, and bounded task packets, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
`agent-tools.json` is static agent tool catalog v4. It retains exact action and current-state resource bindings and adds the bounded authenticated task-packet binding. Resource routes reuse exact query inputs and enforcement, return only the validated query result inside resource envelope v1, and preserve conditional disclosure, bounds, pagination, and private read evidence. Freshness is deliberately conservative: point-in-time retrieval, zero reusable lifetime, revalidation before reuse, and `Cache-Control: no-store`. The envelope does not echo input or authenticated identity and grants no authority.

`mcp.json` is MCP adapter manifest v2. It exposes every catalog action and query as a stable MCP tool with the catalog's exact schemas and exposes `modellang_task_packet` as a distinct read-only assembler. The generated server-only `typescript/mcp-server.ts` implements authenticated stateless Streamable HTTP: the host verifies an audience-bound bearer token on every request, and the adapter invokes the existing runtime executor. Query calls additionally return a distinct embedded resource envelope v1; packet calls return task packet v1 as exact structured content and an embedded packet resource. Actions remain commands and do not become resources. Discovery and packet results grant no authority. Resource templates, subscriptions, prompts, MCP Tasks, delegated capabilities, public traces, extension tools, complete task closure, and full SML-Agent conformance are not claimed.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v6 for `target:postgresql-http-ui-agent-task-packets/6`: it marks required core semantics, the static agent catalog, action subject views, current-state query resources, direct MCP adapter, and bounded task packets as native and every extension as an external implementation gap; it grants no authority.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.42 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v6 reports the static catalog, bounded subject action view, query-backed current-state resources, direct MCP adapter, and explicitly partial task packets as native. Automatic observation selection, complete effect/recovery closure, MCP resource templates and subscriptions, prompts, MCP Tasks, delegated capabilities, public policy traces, extension-backed tools, adversarial agent testing, host implementation verification, alternate target generators, cross-target conformance, nonzero freshness lifetimes, read-evidence observation, and general recovery workflows remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.42 language is in [spec/0.42/LANGUAGE.md](./spec/0.42/LANGUAGE.md), with its [bounded task-packet contract](./spec/0.42/AGENT_TASK_PACKETS.md), [conformance requirements](./spec/0.42/CONFORMANCE.md), and [unstable boundaries](./spec/0.42/UNSTABLE.md). Earlier MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.42 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
