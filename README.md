# ModelLang 0.43 reference compiler
...
`agent-tools.json` is static agent tool catalog v5. It retains exact action, current-state resource, and bounded task-packet bindings and adds delegated capability v1 management and invocation metadata. Resource routes reuse exact query inputs and enforcement, return only the validated query result inside resource envelope v1, and preserve conditional disclosure, bounds, pagination, and private read evidence. Freshness is deliberately conservative: point-in-time retrieval, zero reusable lifetime, revalidation before reuse, and `Cache-Control: no-store`. Catalog discovery and resource envelopes grant no authority.

`mcp.json` is MCP adapter manifest v3. It exposes every catalog action and query as a stable MCP tool with the catalog's exact schemas and exposes `modellang_task_packet` as a distinct read-only assembler. The generated server-only `typescript/mcp-server.ts` implements authenticated stateless Streamable HTTP: the host verifies an audience-bound bearer token on every request, and the adapter invokes the existing runtime executor. Query calls additionally return a distinct embedded resource envelope v1; packet calls return task packet v1 as exact structured content and an embedded packet resource. An action may also carry the separately issued `_meta["dev.modellang/delegatedCapability"]` credential while retaining ordinary delegate authentication. Issuance remains HTTP-only. Actions remain commands and do not become resources. Discovery and packet results grant no authority. Resource templates, subscriptions, prompts, MCP Tasks, public traces, extension tools, complete task closure, and full SML-Agent conformance are not claimed.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v7 for `target:postgresql-http-ui-delegated-capabilities/7`: it marks required core semantics, the static agent catalog, action subject views, current-state query resources, direct MCP adapter, bounded task packets, and the delegated-capability adapter contract as native while naming the host credential authority in enforcement; every extension remains an external implementation gap.
...
The generated TypeScript clients expose only declared actions, queries, action applicability, the bounded subject capability overlay, and query-backed agent resources. They have no generic table or mutation API. Caller identity is not an input field.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.43 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v7 reports the static catalog, bounded subject action view, query-backed current-state resources, direct MCP adapter, explicitly partial task packets, and the bounded delegated-capability adapter contract as native. The host credential authority remains required and is not generated or attested. Automatic observation selection, complete effect/recovery closure, MCP resource templates and subscriptions, prompts, MCP Tasks, transferable or chained delegation, public policy traces, extension-backed tools, adversarial agent testing, host implementation verification, alternate target generators, cross-target conformance, nonzero freshness lifetimes, read-evidence observation, and general recovery workflows remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.43 language is in [spec/0.43/LANGUAGE.md](./spec/0.43/LANGUAGE.md), with its [delegated-capability contract](./spec/0.43/DELEGATED_CAPABILITIES.md), [conformance requirements](./spec/0.43/CONFORMANCE.md), and [unstable boundaries](./spec/0.43/UNSTABLE.md). Earlier task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.43 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
