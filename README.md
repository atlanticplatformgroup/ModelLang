# ModelLang 0.39 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static agent tool catalog with authenticated subject-specific action views, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
`agent-tools.json` is static agent tool catalog v2. It projects every declared action and query into a stable tool identity with standalone closed JSON input/output schemas, exact authenticated HTTP execution routes, failure classes, read-only annotations, action applicability routes and safe rule IDs, reliability and emitted-event declarations, and bounded query metadata. It also declares `POST /agent/capabilities`, where an authenticated caller can submit up to 32 distinct exact action candidates and receive an input-specific available/unavailable overlay. The response excludes caller identity, candidate input, resource state, expressions, extensions, implementation locations, and runtime internals. Queries remain static-catalog-only. The catalog can feed an MCP tool adapter, but it is not an MCP server, task packet, delegated capability, current-state resource contract, or SML-Agent conformance claim; every later execution re-enforces current runtime authority.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v3 for `target:postgresql-http-ui-agent-subject-view/3`: it marks required core semantics, the static agent catalog, and action subject views as native and every extension as an external implementation gap; it grants no authority.
...
The generated TypeScript clients expose only declared actions, queries, action applicability, and the bounded subject capability overlay. They have no generic table or mutation API. Caller identity is not an input field.

- `typescript/browser.ts` is the browser-safe entry point. Its HTTP client sends JSON to stable-ID execution and applicability routes plus the authenticated subject capability route, and its UI executor keeps assessment separate from execution. It contains no SQL, database adapter, Node.js, or PostgreSQL contract.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.39 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, and opt-in committed private read evidence.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v3 reports the static catalog and bounded subject action view as native. Extension invocation, host implementation verification, alternate target generators, cross-target conformance, public policy traces, current-state resources, freshness lifetimes, read-evidence observation, general recovery workflows, and direct MCP generation remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.39 language is in [spec/0.39/LANGUAGE.md](./spec/0.39/LANGUAGE.md), with its [subject capability view contract](./spec/0.39/SUBJECT_CAPABILITY_VIEWS.md), [conformance requirements](./spec/0.39/CONFORMANCE.md), and [unstable boundaries](./spec/0.39/UNSTABLE.md). Earlier agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.39 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
