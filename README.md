# ModelLang 0.47 reference compiler
...
Each model has a generated subtree: `generated/procurement/` and `generated/reservations/`. Its `model.ir.json` is the only compiler-backend input. ModelLang 0.47 uses canonical IR1 and adds deployment-configurable, revision-bound caching for static MCP discovery without changing source-language, IR, authority, runtime-operation, or current-state freshness semantics. Evolution tools accept only the current IR format; migrations still compare two current-format artifacts by stable identity. Both committed subtrees are golden fixtures and migration baselines.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero state freshness lifetimes, as-of reads, public audit observation, full-text search, and model-authored cache policy. ModelLang 0.47 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes. Its deployment-only MCP discovery TTL applies only to static server/tool descriptions.
...
The normative 0.47 language is in [spec/0.47/LANGUAGE.md](./spec/0.47/LANGUAGE.md), with its [MCP discovery-cache contract](./spec/0.47/MCP_DISCOVERY_CACHE.md), [conformance requirements](./spec/0.47/CONFORMANCE.md), and [unstable boundaries](./spec/0.47/UNSTABLE.md). Earlier assurance/evaluation, extension-tool, public-trace, delegated-capability, task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.47 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
