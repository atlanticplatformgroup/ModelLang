# ModelLang 0.49 reference compiler
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero state freshness lifetimes, as-of reads, public audit observation, full-text search, and model-authored cache policy. ModelLang 0.49 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes. Its deployment-only MCP discovery TTL applies only to static server/tool descriptions, while Agent Plugin endpoint/package configuration remains outside the language and IR.
...
The normative 0.49 language is in [spec/0.49/LANGUAGE.md](./spec/0.49/LANGUAGE.md), with its [public preview distribution contract](./spec/0.49/PUBLIC_PREVIEW_DISTRIBUTION.md), [conformance requirements](./spec/0.49/CONFORMANCE.md), and [unstable boundaries](./spec/0.49/UNSTABLE.md). Earlier Agent Plugin packaging, MCP discovery-cache, assurance/evaluation, extension-tool, public-trace, delegated-capability, task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.49 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
