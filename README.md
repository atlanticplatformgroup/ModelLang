# ModelLang 0.44 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static agent tool catalog with authenticated subject-specific action views, current-state query resources, bounded task packets, and exact single-use delegated action capabilities, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
`agent-tools.json` is static agent tool catalog v6. It retains exact action, current-state resource, bounded task-packet, and delegated capability bindings and adds public decision trace v1. Trace requests reuse current action applicability and publish ordered safe rule outcomes without operation inputs, current values, identity, expressions, policy or authority identities, private evidence, or execution claims. Resource routes continue to preserve conditional disclosure, bounds, pagination, and private read evidence. Resources and traces use point-in-time retrieval, zero reusable lifetime, revalidation before reuse, and `Cache-Control: no-store`. Catalog discovery, resources, and traces grant no authority.

`mcp.json` is MCP adapter manifest v4. It exposes every catalog action and query as a stable MCP tool with the catalog's exact schemas, `modellang_task_packet` as a distinct read-only assembler, and `modellang_public_decision_trace` as a separate read-only applicability explanation. The generated server-only `typescript/mcp-server.ts` implements authenticated stateless Streamable HTTP: the host verifies an audience-bound bearer token on every request, and the adapter invokes the existing runtime executor. Query, packet, and trace calls additionally return their exact envelopes as distinct embedded resources. An action may also carry the separately issued `_meta["dev.modellang/delegatedCapability"]` credential while retaining ordinary delegate authentication; traces reject it. Issuance remains HTTP-only. Actions remain commands and do not become resources. Discovery, packets, and traces grant no authority. Resource templates, subscriptions, prompts, MCP Tasks, full or historical traces, extension tools, complete task closure, and full SML-Agent conformance are not claimed.
...
`semantic.json` is engineering semantic manifest v18. It includes the complete prior semantic closure plus every typed extension contract, declared effect, implementation target, governance obligation, and external-only execution marker.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v8 for `target:postgresql-http-ui-public-decision-traces/8`: it marks required core semantics, the static agent catalog, action subject views, current-state query resources, direct MCP adapter, bounded task packets, delegated-capability adapter contract, and bounded public applicability traces as native while naming the host credential authority in delegation enforcement; every extension remains an external implementation gap.

`provenance.json` v2 records compiler, generator, and target profiles, model and IR identity, and the role and SHA-256 content hash of every other generated artifact. It omits wall-clock timestamps and its own recursive hash, so identical compilation inputs remain byte-for-byte deterministic.
...
The generated TypeScript clients expose only declared actions, queries, action applicability, the bounded subject capability overlay, query-backed agent resources, bounded task packets, delegated-capability management, and public applicability traces. They have no generic table or mutation API. Caller identity is not an input field.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.44 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v8 reports the static catalog, bounded subject action view, query-backed current-state resources, direct MCP adapter, explicitly partial task packets, bounded delegated-capability adapter, and public applicability traces as native. The host credential authority remains required and is not generated or attested. Automatic observation selection, complete effect/recovery closure, MCP resource templates and subscriptions, prompts, MCP Tasks, transferable or chained delegation, historical or complete public traces, extension-backed tools, adversarial agent testing, host implementation verification, alternate target generators, cross-target conformance, nonzero freshness lifetimes, read-evidence observation, and general recovery workflows remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.44 language is in [spec/0.44/LANGUAGE.md](./spec/0.44/LANGUAGE.md), with its [public-decision-trace contract](./spec/0.44/PUBLIC_DECISION_TRACES.md), [conformance requirements](./spec/0.44/CONFORMANCE.md), and [unstable boundaries](./spec/0.44/UNSTABLE.md). Earlier delegated-capability, task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.44 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
