# ModelLang 0.38 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static filtered agent tool catalog, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
Each model has a generated subtree: `generated/procurement/` and `generated/reservations/`. Its `model.ir.json` is the only compiler-backend input. ModelLang 0.38 retains IR26 and adds an agent-facing projection of existing operation and capability contracts without changing language semantics or runtime authority. IR9 through IR26 remain accepted evolution baselines; historical models gain neither invented extension obligations nor fabricated evidence. Both committed subtrees are golden fixtures and migration baselines.
...
`agent-tools.json` is static agent tool catalog v1. It projects every declared action and query into a stable tool identity with standalone closed JSON input/output schemas, exact authenticated HTTP execution routes, failure classes, read-only annotations, action applicability routes and safe rule IDs, reliability and emitted-event declarations, and bounded query metadata. It excludes caller identity inputs, expressions, current state, extensions, implementation locations, and runtime internals. The catalog can feed an MCP tool adapter, but it is not an MCP server, an authorization-filtered subject view, a task packet, a delegated capability, or an SML-Agent conformance claim; every execution and applicability request remains authenticated and runtime enforcement is authoritative.
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v2 for `target:postgresql-http-ui-agent-catalog/2`: it marks required core semantics and the static agent catalog as native and every extension as an external implementation gap; it grants no authority.
...
- Engineering semantic manifest v18, extension ledger v1, and target profile v1 are trusted static artifacts, not authorization-filtered capability views. Extension invocation, host implementation verification, alternate target generators, cross-target conformance, public policy traces, freshness lifetimes, read-evidence observation, general recovery workflows, and agent/MCP generation remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.38 language is in [spec/0.38/LANGUAGE.md](./spec/0.38/LANGUAGE.md), with its [agent tool catalog contract](./spec/0.38/AGENT_TOOL_CATALOG.md), [conformance requirements](./spec/0.38/CONFORMANCE.md), and [unstable boundaries](./spec/0.38/UNSTABLE.md). Earlier target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.38 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
