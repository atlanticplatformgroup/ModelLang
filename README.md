# ModelLang 0.46 reference compiler

ModelLang compiles a small domain ontology into an authenticated application boundary backed by PostgreSQL enforcement. The compiler produces a typed canonical IR with persistent semantic identity, reusable closed policies, reliable commands, typed domain events and consumers, one canonical enforcement decision plan, filtered public applicability, a static agent tool catalog with authenticated subject-specific action views, current-state query resources, bounded task packets, exact single-use delegated action capabilities, bounded public applicability traces, host-adapter bindings for declared external extensions, and a conservative SML-Agent assessment, private transactional action and read evidence, explicit external-extension obligations, target capability assessment, engineering policy coverage, a workflow-aware operation and UI boundary, deterministic provenance, guarded evolution, generated clients, and PostgreSQL enforcement.
...
The normative 0.46 language is in [spec/0.46/LANGUAGE.md](./spec/0.46/LANGUAGE.md), with its [agent-assurance contract](./spec/0.46/AGENT_ASSURANCE.md), [evaluation contract](./spec/0.46/AGENT_EVALUATION.md), [conformance requirements](./spec/0.46/CONFORMANCE.md), and [unstable boundaries](./spec/0.46/UNSTABLE.md). Earlier extension-tool, public-trace, delegated-capability, task-packet, MCP, agent-resource, subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.46 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
