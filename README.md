# ModelLang 0.40 reference compiler
...
`extensions.json` is non-public extension ledger v1. It records exact typed external obligations and always reports zero generated implementations. `target-capabilities.json` is target profile v4 for `target:postgresql-http-ui-agent-resources/4`: it marks required core semantics, the static agent catalog, action subject views, and current-state query resources as native and every extension as an external implementation gap; it grants no authority.
...
- Queries intentionally omit inferred joins, reverse or collection traversal, aggregates, compound or nullable sort keys, arbitrary caller-controlled sorting and limits, offset/page-number pagination, runtime-selected fields, redaction reason objects, nonzero freshness lifetimes, as-of reads, public audit observation, full-text search, and cache policy. ModelLang 0.40 retains explicit optional filters, closed authored single-field sort profiles, bounded to-one projection traversal, fail-closed conditional field disclosure, opt-in fixed-limit keyset cursors, opt-in committed private read evidence, and zero-age agent resource envelopes.
...
- Engineering semantic manifest v18 and extension ledger v1 remain trusted static artifacts. Target profile v4 reports the static catalog, bounded subject action view, and query-backed current-state resources as native. Extension invocation, host implementation verification, alternate target generators, cross-target conformance, public policy traces, nonzero freshness lifetimes, read-evidence observation, general recovery workflows, and direct MCP generation remain future contracts.
- Elevated PostgreSQL authorities can bypass the boundary and are intentionally out of scope.

The normative 0.40 language is in [spec/0.40/LANGUAGE.md](./spec/0.40/LANGUAGE.md), with its [agent resource contract](./spec/0.40/AGENT_RESOURCES.md), [conformance requirements](./spec/0.40/CONFORMANCE.md), and [unstable boundaries](./spec/0.40/UNSTABLE.md). Earlier subject-view, agent-catalog, target capability, extension, read evidence, disclosure, sorting, filtering, pagination, traversal, projection, claim, acknowledgement, observation, publication/consumer recovery, failure-disposition, event-chain, reliable-consumer, transactional-event, reliable-command, policy, applicability, reviewed evolution, semantic closure, workflow, UI, gateway, transport, and safe-evolution contracts remain normative where 0.40 does not replace them. The repository edition of [The Semantic Model Layer whitepaper](./docs/whitepaper/THE_SEMANTIC_MODEL_LAYER.md) records demonstrated, partial, and research-stage capabilities.
